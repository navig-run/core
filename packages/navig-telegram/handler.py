...
def on_load(ctx: PluginContext) -> None:
    ...
    try:
        ...
        raise RuntimeError(f"navig-telegram: cannot import telegram_worker — {exc}") from exc
...
def on_event(event: PluginEvent, ctx: PluginContext) -> dict[str, Any] | None:
    ...
    logger.debug("[navig-telegram] on_event: %s from %s (no action)", event.name, event.source)
    return None