...
def cmd_download(args: dict) -> dict:
    ...
    try:
        ...
        return ok(TOOL, "download", {"url": url, "output": output,
                                      "audio_only": audio, "dry_run": dry,
                                      "log": out[-2000:] if out else ""}, ms=t.ms())
...
def cmd_info(args: dict) -> dict:
    ...
    try:
        exe = _ytdlp()
        rc, out, er = run([exe, "--dump-json", "--no-playlist", args["url"]], timeout=30)
        if rc != 0:
        ...
        try:
            ...
            data = {k: meta.get(k) for k in
                    ("id","title","uploader","duration","view_count","upload_date",
                     "description","thumbnail","webpage_url","ext","filesize_approx")}
        except Exception:
            data = {"raw": out[:3000]}
