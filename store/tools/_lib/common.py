    ...
    if is_windows():
        try:
            ...
            for letter in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
                if bitmask & 1:
                    drive = Path(f"{letter}:\\")
                    if ctypes.windll.kernel32.GetDriveTypeW(str(drive)) == 2 and drive.exists():
                        return drive
                bitmask >>= 1
...
def require_usb_root() -> Path:
    r = find_usb_root()
    if not r:
        raise RuntimeError("USB root not found. Connect the USB drive or set NAVIG_USB_ROOT.")
    return r
...
_TOOL_EXES: dict[str, dict[str, str | None]] = {
    ...
    "rclone":       {"windows": "network/rclone-browser/rclone/rclone.exe",
    ...
    "nvidia_check": {"windows": "system/nvidia-updater/TinyNvidiaUpdateChecker.exe", "linux": None, "mac": None},
    "futurerestore":{"windows": "ios/futurerestore/futurerestore.exe",          "linux": None, "mac": None},
}

# System tools (not USB-resident) — resolved from PATH
_SYSTEM_TOOLS = {"rclone_sys", "iperf3_sys", "yt-dlp", "gh", "scrot", "screencapture", "gnome-screenshot"}
...
def ok(tool_id: str, command: str, data: Any = None,
       warnings: list[str] | None = None, ms: int = 0,
       backend: str = "worker") -> dict:
    return {
        "ok": True, "tool": tool_id, "command": command,
        "ts": datetime.now(timezone.utc).isoformat(),
        "data": data or {}, "warnings": warnings or [], "errors": [],
        "metrics": {"ms": ms, "backend": backend},
    }

def err(tool_id: str, command: str, errors: list[str],
        warnings: list[str] | None = None, ms: int = 0,
        backend: str = "worker") -> dict:
    return {
        "ok": False, "tool": tool_id, "command": command,
        "ts": datetime.now(timezone.utc).isoformat(),
        "data": {}, "warnings": warnings or [], "errors": errors,
        "metrics": {"ms": ms, "backend": backend},
    }
...
def run(args: list[str | Path], timeout: int = 60,
        dry_run: bool = False, env: dict | None = None) -> tuple[int, str, str]:
...
class Timer:
    ...
    def ms(self) -> int: return int((time.monotonic() - self._start) * 1000)
