...
def _check(): return "futurerestore is Windows-only in this config." if current_os() != "windows" else None

def cmd_restore(args: dict) -> dict:
    t = Timer()
    if e := _check(): return err(TOOL, "restore", [e], ms=t.ms())
    try:
        blob = args["blob"]
        ipsw = args["ipsw"]
        ...
        if not dry:
            for f, label in [(blob, "--blob"), (ipsw, "--ipsw")]:
                if not Path(f).exists():
                    return err(TOOL, "restore", [f"{label} file not found: {f}"], ms=t.ms())
        exe = resolve_usb_exe("futurerestore")
        cmd = [exe, "--use-pwndfu", "-t", blob, ipsw]
        if dry:
            return ok(TOOL, "restore", {"dry_run": True, "cmd": [str(c) for c in cmd]}, ms=t.ms())
