...
def cmd_ls(args: dict) -> dict:
    ...
    try:
        ...
        for l in lines:
            ...
            files.append({"size": int(parts[0]) if parts[0].isdigit() else 0,
                           "name": parts[1] if len(parts) > 1 else parts[0]})
        return ok(TOOL, "ls", {"remote": target, "files": files, "count": len(files)}, ms=t.ms())
...
def cmd_sync(args: dict) -> dict:
    ...
    try:
        ...
        return ok(TOOL, "sync", {"source": args["source"],
                                  "destination": args["destination"],
                                  "output": out, "dry_run": args.get("dry_run", False)}, ms=t.ms())
    except Exception as e:
...
def cmd_copy(args: dict) -> dict:
    ...
    try:
        ...
        return ok(TOOL, "copy", {"source": args["source"],
                                  "destination": args["destination"],
                                  "output": out}, ms=t.ms())
    except Exception as e:
...
def cmd_remotes(args: dict) -> dict:
    ...
    try:
        ...
        return ok(TOOL, "remotes", {"remotes": remotes, "count": len(remotes)}, ms=t.ms())
    except Exception as e:
