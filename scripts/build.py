...
def measure_python_startup(iterations: int = 5) -> float:
    ...
    for _ in range(iterations):
        start = time.perf_counter()
        subprocess.run([sys.executable, "-m", "navig", "--help"], capture_output=True, cwd=PROJECT_ROOT)
...
def build_nuitka(onefile: bool = True) -> Optional[Path]:
    ...
    try:
        code, stdout, _ = run_command([sys.executable, "-m", "nuitka", "--version"], capture=True)
        if code == 0:
            print(f"  Nuitka version: {stdout.strip().split()[-1] if stdout else 'unknown'}")
        else:
            raise ImportError()
    except Exception:
...
def compare_builds():
    ...
    if pyinstaller_bin and pyinstaller_bin.exists():
        ...
        results["pyinstaller"] = {"startup_ms": startup, "size_mb": size, "path": pyinstaller_bin}
    ...
    if "pyinstaller" in results and "nuitka" in results:
        ...
        if nu["size_mb"] < pi["size_mb"] and nu["startup_ms"] < pi["startup_ms"]:
            print("  Nuitka produces smaller, faster binaries. Recommended for production.")
        elif pi["size_mb"] < nu["size_mb"]:
            print("  PyInstaller produces smaller binaries. Consider for size-constrained deployments.")
        else:
...
def measure_startup():
    ...
    code, stdout, _ = run_command([
        sys.executable, "-X", "importtime", "-c", "import navig.cli"
    ], capture=True)
...
Examples:
  ...
  python scripts/build.py --measure-startup     Analyze startup performance
    ...
    parser.add_argument(
        "--compare",
        action="store_true",
        help="Build with both tools and compare"
    )
    parser.add_argument(
        "--compile-bytecode",
        action="store_true",
