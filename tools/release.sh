#!/usr/bin/env bash
# tools/release.sh — Create and push a signed annotated release tag.
#
# Usage: bash tools/release.sh <version>
# Example: bash tools/release.sh 2.5.0
#
# Requirements:
...
VERSION="${1:-}"
if [[ -z "$VERSION" ]]; then
  echo "Usage: bash tools/release.sh <version>"
  echo "Example: bash tools/release.sh 2.5.0"
  exit 1
fi
