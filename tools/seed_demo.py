...
NAVIG_CONFIG_DIR. For an isolated scratch space:
    ...
    NAVIG_DATA_DIR=/tmp/navig-demo python tools/seed_demo.py

Usage
-----
    python tools/seed_demo.py            # seed (skips rows that already exist)
    python tools/seed_demo.py --reset    # remove all demo rows
    python tools/seed_demo.py --dry-run  # show the target paths + plan, write nothing
"""
...
# Allow running straight from the repo: `python tools/seed_demo.py`
...
def main() -> None:
    ...
    if not args.dry_run:
        ...
        print("Undo with: python tools/seed_demo.py --reset")
