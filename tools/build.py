...
Usage:
    python tools/build.py --tool pyinstaller    # Build with PyInstaller
    python tools/build.py --tool nuitka         # Build with Nuitka
    python tools/build.py --compare             # Benchmark both tools
    python tools/build.py --compile-bytecode    # Pre-compile .pyc files
    python tools/build.py --measure-startup     # Measure startup time
"""
...
def run_command(
    ...
    Captured as bytes and decoded UTF-8-first, console-page-second, rather than with
    `text=True` — which uses the ANSI code page and is wrong for both the UTF-8 tooling
    this drives (uv, python -m build) and any Windows console tool it may shell out to.

    Deliberately NOT importing `navig.core.proc_text`: this is the script that BUILDS
    navig and must keep running when navig is not importable. Four lines duplicated to
    stay dependency-free is the right trade for a bootstrap script.
    """
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python tools/build.py --tool pyinstaller    Build single binary with PyInstaller
  python tools/build.py --tool nuitka         Build optimized binary with Nuitka
  python tools/build.py --compare             Build both and compare
  python tools/build.py --compile-bytecode    Pre-compile Python bytecode
  python tools/build.py --measure-startup     Analyze startup performance
        """,
    )
