...
### Fixed
- **`install.sh --dry-run` no longer fails on hosts without pip**: The script previously continued through dependency detection and exited with `pip not available`, even in preview mode. `main()` now short-circuits early in dry-run mode after rendering a non-mutating preview (`Action` + `Dry run complete`) and exits successfully.
