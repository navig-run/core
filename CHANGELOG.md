...
- **Perplexity AI outbound connector (`navig/connectors/perplexity/`)**: New `PerplexityConnector` integrating Perplexity Sonar API as a search-only connector. Domain: `AI_RESEARCH`. Auth: `PERPLEXITY_API_KEY` env var (no OAuth, no vault dependency). `search()` POSTs to `https://api.perplexity.ai/chat/completions` (model: `sonar`) and returns a `Resource` for the synthesized answer plus citation-URL resources. `health_check()` measures API latency. `fetch()` and `act()` return not-supported values as per connector contract. Registered in `connector_cmd.py` auto-discovery. 17 tests in `tests/connectors/test_perplexity.py`.
- **Connector Engine (Phase 1 — Gmail + Google Calendar)**: New `navig/connectors/` package providing a unified integration layer for external services. Core infrastructure: `BaseConnector` ABC with `search`/`fetch`/`act`/`health_check` contract, `ConnectorRegistry` thread-safe singleton, `ConnectorManifest` metadata, `CircuitBreaker` per-connector resilience, `ConnectorAuthManager` vault+OAuth PKCE bridge. Shared types: `ConnectorDomain` (13 domains), `ConnectorStatus`, `Resource`, `Action`/`ActionResult`, `HealthStatus`, `ResourceType` enums+dataclasses. Error hierarchy: `ConnectorError` → `ConnectorAuthError`, `ConnectorNotFoundError`, `ConnectorDegradedError`, `ConnectorAPIError`, `ConnectorRateLimitError`. Phase 1 connectors: **Gmail** (`navig/connectors/gmail/`) — search, fetch, send/reply/archive/label/delete via Gmail API v1; **Google Calendar** (`navig/connectors/google_calendar/`) — search, fetch, create/update/delete events, conflict checking via Calendar API v3. **SmartLinker** cross-connector relationship engine links Gmail↔Calendar via date mentions and attendee overlap. CLI commands: `navig connector list|status|connect|disconnect|search|fetch|health`. 62 tests covering all modules.
- **Vault-backed Telegram owner identity (`telegram.user_id`)**: `navig/messaging/secrets.py` — new `resolve_telegram_uid(raw_config=None) → str | None` and `ensure_telegram_uid(vault, raw_config) → str` following the exact pattern of `resolve_telegram_bot_token`. Resolution order: vault v2 labels (`telegram/user_id`, `telegram.user_id`, `telegram_user_id`) → vault v1 (`telegram/user_id` key) → `NAVIG_TELEGRAM_UID` env var → `~/.navig/.env` → `telegram.user_id` in config (deprecated, logs warning). `ensure_telegram_uid` prompts once on first interactive run, stores permanently to vault v2, and raises `RuntimeError` in headless/CI mode with a clear `NAVIG_TELEGRAM_UID` instruction. Vault write failure is fatal (does not silently proceed with unsaved UID). `navig/daemon/telegram_worker.py` and `navig/messaging/registry.py` `_telegram_config()` now include `owner_uid` in the returned dict. `navig/onboarding/steps.py` `_step_telegram_bot()` prompts for the owner Telegram user ID after a successful token save and stores it to vault v2 + `.env`. `navig/commands/vault.py` `vault_set`/`vault_get` now dual-write dot-notation keys to vault v2 so `navig vault set telegram.user_id <uid>` and `navig vault get telegram.user_id` round-trip correctly via the v2-first resolution chain. `tests/test_telegram_uid_resolution.py` — 10 tests covering all resolution paths, precedence rules, headless guard, happy-path vault write, and vault-write failure.

 in `pyproject.toml` and added to `requirements.txt`. Users who installed with bare `pip install navig` or the official installers previously received only the numbered fallback picker (Tier 3); arrow-key navigation (Tier 2) now works out of the box. The `navig[interactive]` extra is preserved as a documented alias. `install.ps1` and `install.sh` updated to install `navig[interactive]` so the extra is always resolved regardless of install path.
- **Optional fzf install in official installers**: `install.ps1` now tries `winget install junegunn.fzf` after `navig` is installed; prints a manual hint if winget is unavailable. `install.sh` tries `brew`/`apt-get`/`pacman`/`dnf` in order based on detected OS; prints a manual hint if no package manager succeeds.
- **Picker degradation hints**: `navig/cli/selector.py` now prints one-time per-session hints to stderr on interactive TTYs when falling through tiers — `"Tip: install fzf …"` when falling from Tier 1 to 2, `"Tip: pip install navig[interactive]"` when falling from Tier 2 to 3. `navig/onboarding/steps.py` `_step_core_navig` prints a fzf install hint when `fzf` is absent at `navig init` time.

- **Automatic Nerd Font detection and terminal capability persistence**: `navig/ui/_capabilities.py` — new module with `probe_nerd_font()` (Windows: `winreg` + font-dir glob; macOS: `~/Library/Fonts`; Linux: `~/.local/share/fonts` + `fc-list` fallback), `read_terminal_json`/`write_terminal_json` for `~/.navig/terminal.json` caching, and `try_install_nerd_font()` (calls `Install-NerdFont.ps1` via `pwsh` on Windows; prints one-liner on macOS/Linux). `navig/ui/theme.py`: new `NERD_FONT_AVAILABLE` bool (env override `NAVIG_NERD_FONT=1/0` → `terminal.json` cache → active probe; skipped in CI/NO_COLOR). `navig/ui/icons.py`: `nf_icon()` now guards on `NERD_FONT_AVAILABLE` and falls back to `icon()` on unconfigured terminals. `navig/onboarding/steps.py`: new `terminal-setup` bootstrap step (phase 1, `on_failure="skip"`) that probes at `navig init` time, offers a Y/n install prompt on interactive TTYs, and writes `terminal.json` in all paths. `install.ps1`: `Write-TerminalCapabilities` writes `terminal.json` after `Initialize-NavigConfig`. `install.sh`: `_write_terminal_json` writes `terminal.json` after the install marker.
- **Cross-platform Nerd Font integration**: `scripts/Install-NerdFont.ps1` — idempotent PS7.2+ installer; auto-detects JetBrainsMono Nerd Font, downloads from GitHub releases, installs to OS-appropriate font directory (C:\Windows\Fonts, ~/Library/Fonts, ~/.local/share/fonts), patches VS Code `settings.json`, and patches Windows Terminal `settings.json` on Windows. `scripts/navig-icons.psm1` — single-source Nerd Font glyph registry (`$NAVIG` hashtable, 24 keys) with UTF-8 console encoding guard and `[char]::ConvertFromUtf32()` for supplementary codepoints. `scripts/navig-colors.psm1` — semantic ANSI color wrappers (`Set-NavColor`) with Windows `ENABLE_VIRTUAL_TERMINAL_PROCESSING` guard via P/Invoke. `scripts/navig-statusbar.psm1` — `Show-NavStatus` Powerline status bar renderer with platform OS-icon injection (Windows/macOS/Linux). Python: `navig/ui/icons.py` extended with `_NF_ICONS` dict (24 Nerd Font codepoints) and `nf_icon()` function that falls back to `icon()` for unknown keys.

### Changed
- **Emoji-to-icon registry migration**: Replaced all hardcoded emoji literals covered by the spec in `navig/console_helper.py` (`confirm_operation()` — `⚠️`, `📝`, `ℹ️` replaced via lazy `_ni` import to avoid startup cost), `navig/gateway/channels/telegram_commands.py` (~25 occurrences: bridge status, model tier display, provider picker buttons, voice/focus/session status, debug mode, warn messages), and `packages/telegram-bot-navig/plugins/nlp_aliases.py` (all 10 `_ACTIONS["emoji"]` values + error message). All glyphs now resolve through the `navig.ui.icons` registry with `SAFE_MODE` ASCII fallbacks.

### Internal / Refactor
- **`navig/cli/middleware.py` extracted from `__init__.py`**: New module with four helpers called from `main()` — `init_operation_recorder()`, `init_debug_logger()`, `register_fact_extraction()`, `init_proactive_assistant()`. `__init__.py` reduced from 1491 → 1250 lines (−241). No behaviour change; 39/39 regression tests pass.
- **AI command surface rationalisation (Phase 1)**: Reduces 7+ overlapping AI entry points to a clean two-command surface (`navig ask` + `navig agent`). Changes: (1) `navig ai ask`, `navig ai explain`, `navig ai suggest`, `navig ai diagnose` are deprecated — each prints a `deprecation_warning()` redirecting to `navig ask`. (2) `navig chat` is deprecated → `navig ask`; command is unhidden so the warning appears in `--help`. (3) `navig explain` removed from `_EXTERNAL_CMD_MAP` (all three sub-commands were `ch.warn("not yet implemented")` stubs — nothing removed for users). (4) `navig brain` registered in `_EXTERNAL_CMD_MAP` — the prompt-file management module (`brain.py`, 201 lines) was fully implemented but unreachable; it is now accessible as `navig brain prompts list/get/set/reload`. AI management commands (`navig ai models`, `providers`, `airllm`, `login`, `logout`, `memory`) are unchanged.
- **AI intent preview controls on `navig ask` (Phase 2, non-breaking)**: `navig ask` now infers mode via `navig.routing.detect.detect_mode()` and injects route metadata (`ask_detected_mode`, `ask_detected_confidence`, `ask_detected_reasons`) into the ask options payload. New flags: `--plan` prints inferred route and exits without executing; `--plan --execute` prints route then executes. Default `navig ask` behavior is unchanged.

### Fixed
- **Ruff style/correctness cleanup (700+ violations across 50+ files)**: Seven violation categories eliminated: (1) **E741** — renamed 12 ambiguous `l` loop variables to `ln`/`layout`/`lnk`/`lbl`/`item` across `agent/plan_execute.py`, `agents/inbox_router.py`, `commands/agent.py`, `commands/ahk.py`, `commands/links.py`, `commands/monitoring.py`, `deploy/history.py`, `gateway/channels/task_card.py`, `local_operations.py`, `onboarding/steps.py`, `workspace.py`; (2) **W293/W191/E731** — blank-line whitespace (`core/execution.py`), tab→space in `migrations/__init__.py`, lambda→def in `providers/__init__.py`; (3) **I001/SIM114/UP034/C420** — 14 auto-fixes (import sort, collapsed if-arms, redundant parens, dict-comprehension); (4) **B904** — added `raise … from None/exc` to 12 bare re-raise sites in `agent/mcp_client.py`, `cli/recovery.py`, `commands/host.py`, `commands/import_cmd.py`, `commands/telegram.py`, `commands/vault.py`, `commands/webdash.py`; (5) **F811** — removed duplicate `msg_trace_path()` in `platform/paths.py`, redundant `Path` re-import in `commands/skills.py`; (6) **B023** — added `# noqa: B023` for intentional semaphore closure in `agent/conversational.py`; (7) **S101/S602** — `# noqa` for runtime invariant asserts in `agent/mcp_client.py` IPC guards/`agent/tools/skill_tools.py` context guards, and intentional `shell=True` subprocess in `commands/remote.py`/`remote.py`.
- **DRY async bridge `navig/commands/_async_utils.py`**: Extracted duplicated `_run()` async-to-sync helper from `ask.py` and `connector_cmd.py` into a shared `run_sync()` function. Both commands now import `run_sync as _run` from the shared module.
- **`Optional[X]` → `X | None` modernization (PEP 604)**: Updated 15+ files across `cli/middleware.py`, `connection_pool.py`, `commands/onboard.py`, `core/shared_config.py`, `desktop/tray_app.py`, `gateway/deck/__init__.py`, `installer/__init__.py`, `mcp/tools/__init__.py`, `mesh/sync_manager.py`, `messaging/secrets.py`, `modes/manager.py`, `onboarding/telemetry.py`, `plugins/__init__.py`, `tools/sandbox.py`, `tunnel.py`, `webhooks/signatures.py`, `yaml_utils.py` to use Python 3.10+ union-type syntax.
- **Lazy logger formatting**: Replaced f-string arguments in `logger.*()` calls with `%s` placeholder pattern (avoids formatting cost when log level is disabled) across `workspace.py`, `webhooks/receiver.py`, `tasks/worker.py`, `commands/host.py`, `mesh/sync_manager.py`.
- **Ruff F841 dead variable cleanup (36 violations across 35 files)**: Removed dead local variable assignments across agent, browser, gateway, commands, core, memory, onboarding, selfheal, tools, tui, tunnel, ui, and voice modules. Removed: `port`/`host` (ears.py stub), `tagged_count` (prompt_caching.py), `navig_cmd` (service.py), `result=` (imap_email.py — bare call preserved), `system_prompt` (browser/orchestrator.py), `vault=` (vault_bootstrap.py — bare call preserved), `thinking=` (telegram_refiner.py — bare await preserved), `elapsed`/`t0`/`import time` (telegram.py), `model_name` (telegram_keyboards.py → `_`), `file_path` (config_watcher.py), `navig_path`/`cutoff`/`timestamp_str` (commands/agent.py), `fork_url` (contribute.py — bare `fork_repo()` call preserved), `config` (history.py + main.py), `ops` (insights.py), `resp` (matrix.py + matrix_admin.py), `temp_options` (monitoring.py), `password=` (proactive.py — bare prompt preserved), `context` dict (ahk.py), `response=` (ai.py), `event_key` (plugins.py), `fg_off` (genesis.py), `url_len` (tools/web.py), `channels_dir` (tui/resolvers.py), `remote_host` copy-paste bug (tunnel.py), `ico_ai` (ui/summary.py), `stt` (voice/pipeline.py). Cascade-fixed 6 F401 imports (`timedelta`, `get_config_manager`, `Console`, `import time`, `StreamingSTT`). Added `pass` to two empty `if`-bodies. `# noqa: F841` for intentional sentinel/stub patterns (doctor.py, email.py, database_advanced.py, ssh_healer.py, main.py).
- **Ruff F401/F841 lint cleanup (46 violations across 34 files)**: Removed 41 unused imports (`F401`) across 29 files via `ruff --fix` (excluding `navig/cli/__init__.py` re-exports). Manually fixed 5 dead local variable assignments (`F841`): `start_time` in `tools/executor.py`, unused `config_manager` in `commands/config.py:migrate()`, dead `parts`/`cmd_name` in `core/kernel.py` mock loop, `use_shell` in `agent/background_task.py`, and `want_plain` in `commands/docs_cmd.py`.
- **Ruff auto-fix batch (71 issues across 45 files)**: Applied safe auto-fixes for `I001` (unsorted imports), `W293`/`W291` (whitespace on blank lines / trailing whitespace), `F541` (f-strings without placeholders), and `W605` (invalid escape sequences) across the `navig/` source tree and selected test files. Promotes module-level imports and removes dead whitespace produced during prior AI-generated edits.
- **Test isolation — `test_plans_command.py`, `test_spaces_briefing.py`, `test_spaces_next_action.py`**: All three test files converted from `monkeypatch.setattr(Path, "home", ...)` to `monkeypatch.setenv("NAVIG_CONFIG_DIR", str(config_dir))`, matching the `paths.config_dir()` resolution path used by `navig/spaces/resolver.py`, `navig/plans/context.py`, and related modules. Previously these tests created space/plan directories under `home / ".navig" / ...` which was never discovered because the source code reads from `NAVIG_CONFIG_DIR`, not `Path.home()`. All 65 tests across the three suites now pass.
- **Telegram providers screen — unconfigured cloud providers now show `🔑` stub rows**: `_handle_providers()` in `navig/gateway/channels/telegram_commands.py` previously skipped all unready providers, showing only "ℹ️ No configured providers found." Changed to: (1) when a cloud provider has `requires_key=True` and no vault key is stored (`vault_has_key=False`), a single-button stub row `{emoji} {DisplayName} 🔑` with `callback_data=prov_cfg_{id}` is added to the keyboard — inviting the user to configure the key; (2) when a vault key exists but is not validated, the provider is still hidden (no stub) to avoid confusion; (3) the "no ready providers" informational line changed to "ℹ️ No vault-backed cloud providers are ready." for clarity. All 37 `test_telegram_reminders.py` tests pass.
- **`SessionStore._generate_session_id()` collision guard**: Promoted `import secrets` and `import threading` to module level; added `_SESSION_ID_LOCK`, `_SESSION_ID_LAST_TS`, `_SESSION_ID_COUNTER` module-level state so IDs within the same second are monotonically incremented (wrapping at 0xFFFF) instead of random — preventing collision under high-concurrency session creation.
- **`ConfigManager` test isolation — legacy file discovery and `global_config_dir` semantics**: `ConfigManager.__init__` (`navig/config.py`) previously set `global_config_dir` unconditionally to `paths.config_dir()` (real `~/.navig`), causing `_get_config_directories()` to never search the explicit `config_dir` supplied in tests. This broke all five legacy-format host/app lookup tests. Fix: `_get_config_directories()` now returns `[app_config_dir, base_dir]` (early exit) when an explicit `config_dir` was passed, providing full isolation without polluting the real `~/.navig`. `global_config_dir` property semantics are unchanged (still driven by `NAVIG_CONFIG_DIR` env var or `~/.navig`). `HostManager.save()` (`navig/core/hosts.py`) was independently saving to `global_config_dir / "hosts"` instead of `hosts_dir` (i.e. `base_dir / "hosts"`); corrected to use `self._config.hosts_dir` so read and write paths are always consistent. 24/24 `test_config.py` + 9/9 `test_wave10_paths.py` pass.
- **Ruff lint cleanup** (`navig/ai.py`, `navig/commands/config.py`, `navig/config.py`, `navig/gateway/channels/telegram.py`, `navig/gateway/channels/telegram_commands.py`): 12 auto-fixed issues — I001 import ordering, B010 `setattr` with constant attribute, F811 redefined `random` import inside function body. Inline `import threading as _thr` / `import logging as _log` in `config.py` background-validation thread promoted to module-level to avoid `UnboundLocalError` when the fast-path cache-hit branch was not taken.
 The script previously continued through dependency detection and exited with `pip not available`, even in preview mode. `main()` now short-circuits early in dry-run mode after rendering a non-mutating preview (`Action` + `Dry run complete`) and exits successfully.
- **`install.sh --action <value>` now validates unsupported actions early**: Unknown values (e.g. `--action bogus`) previously fell through and failed later with unrelated environment errors. `main()` now rejects invalid actions immediately with a clear `invalid action` failure message and non-zero exit.
- **`navig ask` (and any host picker) silently discards fzf selection**: `fzf_or_fallback()` (`navig/cli/selector.py`) built its reverse-lookup map from the raw padded display strings (e.g. `"o2switch                   "`) but compared against fzf's `.strip()`ped stdout (`"o2switch"`). When `description` is empty the entry ends with trailing whitespace that fzf strips, so `index_map.get()` always returned `None` and `require_active_server` / `require_active_host` raised `typer.Exit(0)` even after a valid selection — causing the picker to re-appear on every command. Fixed by building `index_map` with stripped keys; entries with non-empty descriptions are unaffected.
 - **`navig ask` now resolves active host per invocation and avoids localhost auto-bootstrap bias**: `ask_ai` now resolves host from CLI options + active server + compatibility `active_host` key and calls `require_active_server(..., allow_local_bootstrap=False)` when resolution is still empty. This prevents silent localhost pivoting in ask flow while preserving existing bootstrap behavior for other commands. Missing OpenRouter key errors in ask flow are now host-aware and include source checks (`OPENROUTER_API_KEY`, `ai.api_key`, `openrouter_api_key`) with actionable setup guidance.
- **`navig ask` host guards and options safety**: `ask_ai` now tolerates `options=None`, validates resolved host existence before loading config, and converts missing/invalid host config conditions into clear `Exit(2)` user guidance instead of low-signal exceptions.
- **Host launcher non-TTY guard restored**: `smart_launch()` now requires both `stdin` and `stdout` to be TTY before opening the interactive selector. In subprocess/non-interactive contexts it now consistently prints the `Non-TTY detected` hint and exits cleanly with status 0 (`navig host --help` guidance), preventing accidental interactive picker rendering in CI/tests.
- **Telegram provider/model selection UX overhaul**: (1) All providers now display in a uniform 2-per-row grid (ready providers show ✅, unconfigured show 🔑). (2) Clicking any provider opens the model picker — providers without API keys show a ⚠️ banner instead of being dead buttons. (3) Static `picker_map` dict replaced with dynamic registry lookup so new providers work automatically. (4) Individual model and bulk-activate assignments now update the primary LLM Mode Router (persisted to config) in addition to the optional hybrid router, so `/models` and `/providers` displays reflect changes immediately. (5) `/models` is now a read-only status view — tier switching is done exclusively via `/providers`.
- **`llama.cpp` onboarding now asks for local server URL**: Selecting `llama.cpp` in `navig init` (`ai-provider` step) now prompts for endpoint URL (default `http://127.0.0.1:8080`) instead of silently completing as a generic local provider. The entered value is persisted under `llm_router.provider_base_urls.llamacpp` in `.navig/config.yaml` for reuse.
- **`navig init --provider` now correctly re-runs the `ai-provider` step** (#79): Previously, `_already_completed()` fired for the `jump_to_step` target regardless of whether the caller explicitly asked to re-run it. Because `~/.navig/onboarding.json` preserved the prior `ai-provider: completed` record (and `force=False` when using `--provider`), the step was silently skipped and the engine fell through to `web-search-provider`. The guard in `OnboardingEngine.run()` (`navig/onboarding/engine.py`) now carves out the explicit jump target: `is_jump_target = bool(self._config.jump_to_step) and step.id == self._config.jump_to_step; if not is_jump_target and self._already_completed(step.id): continue`.
- **Progress display during `navig init --provider` no longer shows misleading `[5/18 27%]`** (#79): When `jump_to_step` is active, only the target step and the tail steps execute, so the `[N/total pct%]` format was confusing — the review step (last to run) printed `[5/18 27%]` which looked like an early crash. `run_engine_onboarding()` (`navig/onboarding/runner.py`) now uses `[step N] · Title...` format when `jump_to_step` is set.
...
### Changed
- **Onboarding UX polish — console_helper migration + tests**: Migrated all remaining `sys.stdout.write` calls in `navig/onboarding/runner.py` (progress display, welcome banners, pause/complete messages) and interactive menus in `navig/onboarding/steps.py` (AI provider picker, fallback provider, web-search provider, skills activation, Google SA prompt, review step messages) to `console_helper` (`ch`) for consistent Rich-based output. Added 23 new tests: 17 renderer function tests (`test_onboarding_renderer.py`) covering success/skipped/failure/in-progress display and label resolution, plus 6 review step tests (`test_onboarding_review_step.py`) covering confirm flow, jump-to-step validation, phase-grouped summary, and step metadata. Updated 3 existing tests to mock `ch` module instead of `sys.stdout`.
- **AI provider onboarding source UX upgrade**: `ai-provider` now uses a shared provider source scanner (`env`/`vault`/`config`) for consistent detection, prompts to import environment keys into vault when found, and offers an optional lightweight post-selection connectivity check. `navig init --status` now uses the same scanner to keep provider-source reporting aligned with onboarding.
- **Storage path centralization (phase 1, wave 10)**: Replaced `Path.home() / ".navig"` fallbacks in `navig/config.py` (`ConfigManager.global_config_dir`), `navig/ai.py` (`_get_conv_store()` — migrated to `paths.data_dir()`), `navig/cache_store.py` (`global_cache_dir()` — migrated to `paths.cache_dir()`), `navig/ai_context.py` (`AIContextManager.__init__` default), and in `navig/daemon/supervisor.py`, `navig/daemon/entry.py`, `navig/daemon/service_manager.py` (module-level `NAVIG_HOME` constants). `supervisor._resolve_log_dir()` simplified to a single `paths.log_dir()` call, removing a redundant lazy-import pattern.
- **Storage path centralization (phase 1, wave 9)**: Replaced `Path.home() / ".navig"` fallbacks in `navig/vault/core.py` (`CredentialsVault.DEFAULT_VAULT_PATH`), `navig/memory/user_profile.py` (`_get_memory_dir()`), `navig/memory/snapshot.py` (`_get_snapshot_dir()`), `navig/memory/paths.py` (`navig_home()` — legacy `NAVIG_HOME` preserved, now falls back to `paths.config_dir()`), and `navig/ipc_pipe.py` (`_PROMOTED_FLAG`). Updated docstring examples in memory storage, knowledge_base, indexer, and conversation modules.
- **Storage path centralization (phase 1, wave 8)**: Replaced `Path.home() / ".navig"` fallbacks in `navig/desktop/tray_app.py` (module-level `LOG_DIR`, `NAVIG_DIR` constants and three function-body daemon-path lookups), `navig/inbox/watcher.py` (`_global_inbox_dir()`), `navig/inbox/store.py` (`_inbox_db()`), `navig/identity/store.py` (`get_identity_store()` default), and `navig/gateway/channels/telegram_sessions.py` (`SessionManager.__init__` default). Inbox and SessionManager migrate to `paths.data_dir()`; tray constants use `paths.config_dir()`.
- **Storage path centralization (phase 1, wave 7)**: Replaced `Path.home() / ".navig"` fallbacks in `navig/modes/manager.py` (`_navig_home()`), `navig/onboarding/runner.py` (both `navig_dir` locals), `navig/mcp_manager.py` (`MCPManager.__init__` default), and `navig/providers/auth.py` (`AuthProfileManager.__init__` default) with `paths.config_dir()`.
- **Storage path centralization (phase 1, wave 6)**: `navig/routing/trace.py` (`TRACE_LOG_PATH`) and `navig/perf/profiler.py` (`PERF_DIR`) module-level constants now use `navig.platform.paths.config_dir()` instead of `Path.home() / ".navig" / ...`. Both are relocatable via `NAVIG_CONFIG_DIR`.
- **Storage path centralization (phase 1, wave 5)**: `navig/settings/resolver.py` — replaced the try/except `navig_config_dir()` / `Path.home()/.navig` fallback in `_global_settings_dir()` with a direct `paths.config_dir()` call. The bare `except Exception` escape hatch is removed; `NAVIG_CONFIG_DIR` overrides now propagate correctly through settings resolve, layer lookup, and `SettingsResolver.set()`.
- **Storage path centralization (phase 1, wave 4)**: Gateway runtime defaults now resolve through `navig.platform.paths.config_dir()` in `navig/gateway/audit_log.py` and `navig/gateway/billing_emitter.py`, replacing hardcoded `~/.navig/runtime/*.jsonl` defaults.
- **Storage path centralization (phase 1, wave 3)**: `navig/commands/config.py` now uses `navig.platform.paths.config_dir()` for global config-root resolution (`config validate --scope global`, legacy migration source/target roots, and `config schema install --scope global`).
- **Storage path centralization (phase 1, wave 2)**: `navig/spaces/resolver.py` now resolves global spaces via `navig.platform.paths.config_dir()` instead of hardcoded `~/.navig/spaces`, improving consistency with `NAVIG_CONFIG_DIR` overrides.
- **Storage path centralization (phase 1)**: Replaced hardcoded `~/.navig` lookups with `navig.platform.paths` in startup fast-status context, plugin-cache lookup, and plan-context default space resolution (`navig/main.py`, `navig/plans/context.py`, `navig/agent/tools/plan_tools.py`). This improves relocatability under `NAVIG_CONFIG_DIR` / `NAVIG_DATA_DIR` while preserving legacy plugin-cache fallback behavior.
...
- **CLI dual-registration dead code removal**: Removed ~305 lines of inline `dispatch_app`, `contacts_app`, and `cron_app` definitions from `navig/cli/__init__.py`. All three already had canonical external implementations in `navig.commands.dispatch` and `navig.commands.cron`, registered via `_EXTERNAL_CMD_MAP`. The inline stubs (including `_LazyDispatchGroup`, `_LazyContactsGroup` TyperGroup subclasses and full 7-command `cron_app`) were dead code that could never be reached. File reduced from 9,938 to 9,633 lines. Commands `navig dispatch`, `navig contacts`, and `navig cron` continue working via their external modules.
- **GitHub practices directive**: Added `.github/instructions/github-practices.instructions.md` — authoritative one-PR-per-issue policy, pre-analysis comment requirement, PR body quality standards, issue comment encoding rules (no literal `\n`, no C0 control characters), issue lifecycle labels, and anti-pattern catalogue. Applied to all AI agents and human contributors.
- **Git workflow rule #9**: Added one-issue-per-PR rule to `git.instructions.md` — batch PRs are explicitly forbidden; each fix/feature requires its own branch and a single `Closes #N`.
- **Copilot PR checklist extended**: `copilot-instructions.md` checklist now requires `Closes #N` in PR body, a prior issue analysis comment, and clean comment encoding (real newlines, no C0 chars). Added `## GitHub Practices` reference section.
- **Pull request template overhauled**: `pull_request_template.md` now leads with `## Closes #N` (one issue), adds `## Problem / Root Cause`, `## What Changed`, `## Analysis Reference` sections, and extends the checklist with encoding and one-issue-per-PR items.
- **CLI callback consolidation**: Extracted ~125 lines of inline callback definitions (`show_subcommand_help`, `make_subcommand_callback`, `show_compact_help`, `help_callback`, `version_callback`, `_schema_callback`, `_get_hacker_quotes`) from `navig/cli/__init__.py` into canonical `navig/cli/_callbacks.py`. File reduced from 10,062 to 9,938 lines. All existing `from navig.cli import …` imports continue to work via re-export. Fixed `_callbacks.py` to use `Console(legacy_windows=True)` matching original Windows-safe behavior.
...
- **CLI deprecated-wrapper cleanup**: Removed large inline legacy command wrappers from `navig/cli/__init__.py` (`monitor`, `security`, `system`, `hestia`, `template`, `addon`, `workflow`, plus flat legacy aliases). Canonical command groups remain available through external lazy registration (`navig host monitor`, `navig host security`, `navig web hestia`, `navig flow template`, `navig flow`).
...
- **Init status JSON contract**: `navig --json init --status` now emits pure machine-readable status payload (including readiness issues and next actions) without mixed human-formatted output. `navig --json init` in already-configured state now returns the same payload with `already_configured: true`.
...
- **Telegram `/settings` routing and `/help` formatting fix**: Slash routing now maps `/settings` to the settings hub (`_handle_settings_hub`) instead of the audio panel alias, while `/voice` remains audio-specific. `/help` now renders via deterministic MarkdownV2 command-center output with cleaner sectioning and excludes legacy/admin entries (`/kick`, `/mute`, `/unmute`, `/search`, `/respect`, `/stats_global`) from help text.
- **Telegram MarkdownV2 hardening across interactive flows**: Upgraded key formatted message paths in `telegram_refiner.py`, `telegram_mesh.py`, `telegram_voice.py`, and `telegram_keyboards.py` to `parse_mode="MarkdownV2"` with explicit escaping for user/LLM/provider-derived text in transcripts, Q&A summaries, leader/peer metadata, and audio-file info cards. This prevents raw markdown artifacts and reduces parse failures when content includes special characters.
...
- **Tavily web search provider**: `navig search --provider tavily` is now a first-class runtime provider alongside Brave and DuckDuckGo. Added `TAVILY_SEARCH_ENDPOINT`, `_search_tavily()` (POST-based JSON API, `max_results` capped at 10), Tavily entries in `_WEB_PROVIDER_ALIASES`, `_WEB_PROVIDER_ENV_VARS` (`TAVILY_API_KEY`), and `_WEB_PROVIDER_VAULT_LABELS` (`web/tavily_api_key`). Runtime dispatch updated. 1 new test in `tests/test_web_search_resolution.py`. Tavily also added to the `navig init` wizard (`_WEB_SEARCH_PROVIDER_CATALOG` + `tavily` alias in `_step_web_search_provider`) so users can configure it interactively (#42).
