# Documentation Index

## User Guides

| Document | Description |
|----------|-------------|
| [user/HANDBOOK.md](user/HANDBOOK.md) | Full CLI reference — all commands, flags, examples |
| [user/QUICK_REFERENCE.md](user/QUICK_REFERENCE.md) | Daily cheat sheet for remote ops |
| [user/commands.md](user/commands.md) | Command group overview |
| [user/workflows.md](user/workflows.md) | Common patterns and recipes |
| [user/CLI_COMMANDS.md](user/CLI_COMMANDS.md) | Desktop automation (`navig auto`) |
| [user/INSTALLATION.md](user/INSTALLATION.md) | Installation guide |
| [user/INSTALL_GUIDE.md](user/INSTALL_GUIDE.md) | Detailed install reference |
| [user/troubleshooting.md](user/troubleshooting.md) | Troubleshooting reference |

## Feature Docs

| Document | Description |
|----------|-------------|
| [features/TELEGRAM.md](features/TELEGRAM.md) | Telegram bot integration |
| [TELEGRAM_MANAGER.md](TELEGRAM_MANAGER.md) | Full Telegram organizer — MTProto catalog + bot channel, business catcher, emoji-AI |
| [TELEGRAM_CHEATSHEET.md](TELEGRAM_CHEATSHEET.md) | One-page Telegram · github · tiktok command reference |
| [features/automation.md](features/automation.md) | Automation overview |
| [features/workflows.md](features/workflows.md) | Flow and workflow system |
| [features/wiki.md](features/wiki.md) | Project wiki module |

## Installers and Deployment

| Document | Description |
|----------|-------------|
| [INDEX.md](INDEX.md) | Installer index |
| [deploy/README.md](../deploy/README.md) | Deployment reference |
| [deploy/operational-factory/](../deploy/operational-factory/) | Operational factory stack |

## Developer Docs

| Document | Description |
|----------|-------------|
| [dev/](dev/) | Architecture, contributing, building |
| [upgrade-roadmap.md](upgrade-roadmap.md) | Version upgrade roadmap |
| [STABILIZATION_DEBT.md](STABILIZATION_DEBT.md) | Known technical debt |

Script catalog: [scripts/README.md](../scripts/README.md)
