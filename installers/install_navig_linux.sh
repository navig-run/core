...
build_deck() {
  ...
  local deck_src=""
  local deck_candidates=(
    "${SRC_PATH}/../apps/deck"
    "${SRC_PATH}/../navig-deck"  # dead-path-ok: legacy polyrepo checkout, tried after apps/deck
    "${HOME}/navig/apps/deck"
  )
  for candidate in "${deck_candidates[@]}"; do
