...
def _register_vault_commands():
    ...
    for _cmd in ("vault", "cred", "cred-profile"):  # keep cred/cred-profile so backward-compat path is exercised
...
def test_vault_cli_add_delete(vault):
    ...
    assert result.exit_code == 0
    assert "saved to vault" in result.stdout
    ...
    result = runner.invoke(app, ["vault", "list", "--provider", "openai"])
    assert result.exit_code == 0
    assert "NAVIG Credentials Vault" in result.stdout

    # Delete
    result = runner.invoke(app, ["vault", "delete", cred.id, "--force"])
    assert result.exit_code == 0
...
def test_vault_cli_profile_list():
    result = runner.invoke(app, ["vault", "profile", "list"])
    assert result.exit_code == 0
...
def test_vault_cli_show_nonexistent():
    result = runner.invoke(app, ["vault", "show", "nonexistent"])
    assert result.exit_code != 0
...
def test_vault_cli_activate(vault):
    ...
    runner.invoke(
        app, ["vault", "add", "openai", "--key", "sk-aaa", "--profile", "work", "--no-interactive"]
    )
    runner.invoke(
        app,
        ["vault", "add", "openai", "--key", "sk-bbb", "--profile", "personal", "--no-interactive"],
    )
    ...
    result = runner.invoke(app, ["vault", "activate", work.id[:8]])
...
def test_vault_cli_info_basic(vault):
    ...
    result = runner.invoke(app, ["vault", "info", target.id[:8]])
    assert result.exit_code == 0, result.output
    # Identity section
...
def test_vault_cli_info_reveal(vault):
    ...
    result = runner.invoke(app, ["vault", "info", target.id[:8], "--reveal"])
    assert result.exit_code == 0, result.output
...
def test_vault_cli_info_unknown_id():
    ...
    result = runner.invoke(app, ["vault", "info", "00000000"])
    assert result.exit_code == 1
