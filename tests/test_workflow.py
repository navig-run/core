...
from navig.commands.workflow import (
    Workflow,
    WorkflowStep,
    WorkflowManager,
)
...
class TestWorkflowValidation:
    ...
    def test_validate_missing_command(self):
        ...
        workflow = Workflow(
            name="Test",
            steps=[WorkflowStep(name="Empty Step", command="")]
        )
        errors = manager.validate_workflow(workflow)
...
class TestBuiltinWorkflows:
    ...
    def test_builtin_workflows_valid(self):
        ...
        for name in ["safe-deployment", "db-snapshot", "emergency-debug", "server-health"]:
