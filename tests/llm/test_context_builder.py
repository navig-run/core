...
class TestContextBuilderInternals:
    ...
    def test_load_from_config_yaml_failure_returns_empty_and_logs_debug(self):
        import navig.memory.context_builder as cb_mod

        with (
            patch("navig.config.get_config_manager", side_effect=RuntimeError("boom")),
            patch.object(cb_mod.logger, "debug") as debug_log,
        ):
            result = cb_mod.ContextBuilder._load_from_config_yaml()
