...
class TestGetEnvVarName:
    def test_known_provider_returns_string(self):
        from navig.llm.router import _get_env_var_name
...
class TestInferProviderForUncensored:
    def test_grok_in_alias(self):
        from navig.llm.router import _infer_provider_for_uncensored
...
class TestModeToolsetHints:
    def test_hints_is_dict(self):
        from navig.llm.router import MODE_TOOLSET_HINTS
...
class TestGetLlmRouter:
    def test_returns_llm_mode_router_instance(self):
        from navig.llm.router import LLMModeRouter, get_llm_router
...
class TestExtractUserText:
    def test_extracts_last_user_message(self):
        from navig.llm.generate import _extract_user_text
        msgs = [
...
class TestHasLlmModesConfig:
    ...
    def test_returns_false_when_config_raises(self):
        ...
        with patch("navig.llm.generate.get_config_manager" if hasattr(llm_generate, "get_config_manager") else "navig.config.get_config_manager", side_effect=RuntimeError("no config")):
            from navig.llm.generate import _has_llm_modes_config
