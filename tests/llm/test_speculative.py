...
class TestEndToEnd:
    ...
    def test_mutating_tool_clears_cached_reads(self):
        """Any mutating tool execution must clear speculative read cache."""
        dispatch = MagicMock(return_value="ok")
        exec_ = SpeculativeExecutor(dispatch, config={"enabled": False})

        exec_.cache.put("read_file", {"path": "/tmp/a"}, "cached")
        assert exec_.cache.get("read_file", {"path": "/tmp/a"}) == "cached"

        # Re-add then run a mutating tool; cache must be purged.
        exec_.cache.put("read_file", {"path": "/tmp/a"}, "cached")
        exec_.execute("bash_exec", {"command": "echo hi"})

        assert exec_.cache.get("read_file", {"path": "/tmp/a"}) is None
