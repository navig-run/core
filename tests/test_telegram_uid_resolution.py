...
Coverage:
  - UID in vault v2 only
  - UID in vault v1 only
  - UID in env var only
...
def _stub_v2(uid: str | None):
    """Return a minimal vault-v2 stub whose get_secret returns ``uid``."""
...
def _stub_v1_no_uid():
    """Vault v1 stub that returns None for every get_secret call."""
...
def test_uid_from_vault_v2(monkeypatch):
    mod = _mod()
    monkeypatch.setattr(mod, "_resolve_telegram_uid_from_vault_v2", lambda: "111")
...
def test_uid_from_vault_v1(monkeypatch):
    mod = _mod()
    monkeypatch.setattr(mod, "_resolve_telegram_uid_from_vault_v2", lambda: None)
    monkeypatch.setattr(mod, "_resolve_telegram_uid_from_vault_v1", lambda: "222")
    monkeypatch.setattr(mod, "_resolve_telegram_uid_from_env_file", lambda: None)
    monkeypatch.delenv("NAVIG_TELEGRAM_UID", raising=False)
...
def test_vault_wins_over_config(monkeypatch):
    mod = _mod()
    monkeypatch.setattr(mod, "_resolve_telegram_uid_from_vault_v2", lambda: "vault-uid")
    monkeypatch.setattr(mod, "_resolve_telegram_uid_from_vault_v1", lambda: None)
    monkeypatch.setattr(mod, "_resolve_telegram_uid_from_env_file", lambda: None)
    monkeypatch.delenv("NAVIG_TELEGRAM_UID", raising=False)
