...
class TestAuditRoute:
    ...
    def _make_gw(self, tail_records=None):
        ...
        audit.tail.return_value = tail_records or [
            {"ts": "2026-02-23T10:00:00Z", "actor": "user1", "action": "task.add",  "status": "success"},
            {"ts": "2026-02-23T10:01:00Z", "actor": "user2", "action": "system.stop", "status": "denied"},
            {"ts": "2026-02-23T10:02:00Z", "actor": "user1", "action": "task.add",  "status": "success"},
        ]
        gw.audit_log = audit
        return gw
    ...
    def test_limit_caps_results(self):
        from navig.gateway.routes.audit import _tail
        records = [{"ts": f"T{i}", "actor": "u", "action": "x", "status": "success"} for i in range(20)]
...
class TestRoutePolicyGates:
    ...
    def _gw_with_policy(self, decision_value="deny"):
        from navig.gateway.policy_gate import PolicyGate, PolicyDecision, PolicyResult
        from navig.gateway.audit_log import AuditLog
        from navig.gateway.cooldown import CooldownTracker
        from navig.gateway import server as srv
        gw = MagicMock()
    ...
    def _req(self, gw, body=None):
        ...
        async def _json(): return body or {}
        req.json = _json
        req.match_info = {}
        return req
