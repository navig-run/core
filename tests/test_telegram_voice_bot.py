...
async def test_handle_voice_message(bot):
    with patch("navig.integrations.telegram_voice_bot.TelegramVoiceBot._transcribe") as mock_stt, \
        ...
        context.bot.get_file = AsyncMock(return_value=MagicMock(download_to_drive=AsyncMock(return_value=None)))
