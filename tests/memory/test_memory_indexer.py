...
def test_index_file_logs_debug_on_indexing_exception(tmp_path: Path) -> None:
    storage = _make_storage(tmp_path)
    indexer = MemoryIndexer(storage, embedding_provider=None)
    ...
    indexer._index_file = MagicMock(side_effect=RuntimeError("boom"))  # type: ignore[method-assign]

    with patch("navig.memory.indexer._debug_log") as debug_log:
        result = indexer.index_file(f, base_directory=tmp_path, embed=False)

    assert result.files_failed == 1
    assert result.errors == ["boom"]
    assert any("Failed to index file" in str(call) for call in debug_log.call_args_list)
