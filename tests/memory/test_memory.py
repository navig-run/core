...
class TestAtomicWrite:
    """Tests for atomic write functionality in memory package."""

    def test_atomic_write_retries_permission_error_on_windows(self, tmp_path, monkeypatch):
        """Test that _atomic_write_text retries on PermissionError on Windows."""
        import os
        import sys
        ...
        with patch("time.sleep") as mock_sleep:
            _atomic_write_text(path, '{"test": 123}')

            assert mock_sleep.call_count == 2
            assert path.read_text(encoding="utf-8") == '{"test": 123}'

    def test_manager_add_file_atomic(self, tmp_path):
        from navig.memory.manager import MemoryManager
        
        manager = MemoryManager(memory_dir=tmp_path, use_embeddings=False)
        manager.add_file("content", "test.txt")
