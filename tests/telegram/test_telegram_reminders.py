...
async def test_providers_online_bridge_does_not_override_router_current_provider(monkeypatch):
    ...
    text = bot.messages[-1][1]
    assert "✅ Current: <b>🤖 OpenAI</b>" in text
    assert "✅ Current: <b>⚡ Bridge</b>" not in text
...
async def test_provider_model_picker_is_tier_first_and_edit_in_place(monkeypatch):
    ...
    await bot._show_provider_model_picker(123, "nvidia", page=0, selected_tier="s", message_id=99, show_models=True)
...
async def test_providers_cloud_button_shown_when_vault_present_but_not_yet_validated():
    class _Manifest:
        id = "openai"
        display_name = "OpenAI"
        emoji = "🤖"
    ...
    # vault entry exists (not yet validated): vault_has_key=True → provider is
    # considered ready so the normal activate button is shown (not 🔒 stub).
    bot = _make_provider_bot(
        [_Manifest()],
        verify_fn=_Verify(),
...
async def test_provider_picker_back_button_uses_nav_providers(monkeypatch):
    """Back button in provider picker must use nav:providers, not nav:back."""
    ...
    monkeypatch.setattr("navig.providers.registry._INDEX", {"testprov": _Manifest()})
    monkeypatch.setattr("navig.agent.ai_client.get_ai_client", lambda: _Client())

    bot = _make_dummy_bot()
    await bot._show_provider_model_picker(123, "testprov", page=0, message_id=99, show_models=True)

    assert bot.edits, "Expected an edit_message call"
    keyboard = bot.edits[-1][4]
    all_buttons = [btn for row in keyboard for btn in row]
    back_buttons = [b for b in all_buttons if "Back" in b.get("text", "")]
    assert back_buttons, "Expected a Back button in the keyboard"
    assert all(b.get("callback_data") == "nav:providers" for b in back_buttons), (
        f"Back button must use 'nav:providers', got: {[b.get('callback_data') for b in back_buttons]}"
    )


async def test_provider_picker_falls_back_to_send_when_edit_returns_none(monkeypatch):
    """When edit_message returns None (silent Telegram error), send_message must be used."""
    ...
    bot.edit_message = _failing_edit

    await bot._show_provider_model_picker(123, "testprov", page=0, message_id=88, show_models=True)

    assert bot.edit_message_calls, "edit_message must have been attempted"
    assert bot.messages, (
        "send_message must be called as fallback when edit_message returns None"
    )
