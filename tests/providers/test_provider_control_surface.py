...
class TestSessionOverrides:
    ...
    def test_concurrent_session_metadata_writes_are_consistent(self, tmp_path):
        sm = self._make_manager(tmp_path)

        def _writer(i: int) -> None:
            sm.set_session_metadata(123, 456, f"k{i}", f"v{i}")
        ...
        session = sm.get_or_create_session(123, 456)
        for i in range(40):
            assert session.metadata.get(f"k{i}") == f"v{i}"
...
class TestBugRegressions:
    ...
    def test_vis_clear_no_leading_empty_answer(self):
        ...
        # Read source directly — avoids inspect.getsource returning stale data from
        # module-level state pollution when thousands of tests run before this one.
        src_path = pathlib.Path("navig/gateway/channels/telegram_keyboards.py")
        full_tree = ast.parse(src_path.read_text(encoding="utf-8"))

        method_node = None
        for node in ast.walk(full_tree):
            if isinstance(node, ast.ClassDef) and node.name == "CallbackHandler":
                for item in node.body:
                    if (
                        isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef))
                        and item.name == "_handle_vision_callback"
                    ):
                        method_node = item
                        break
                break
        assert method_node is not None, "_handle_vision_callback not found in CallbackHandler"
        ...
        for node in ast.walk(full_tree):
            if isinstance(node, ast.ClassDef) and node.name == "CallbackHandler":
                for item in node.body:
                    if (
                        isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef))
                        and item.name == "_handle_provider_utility_callback"
                    ):
                        method_node = item
                        break
                break
        assert method_node is not None, "_handle_provider_utility_callback not found in CallbackHandler"
