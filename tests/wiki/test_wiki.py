...
class TestWikiPages:
    ...
    def test_list_with_pages(self, temp_wiki):
        ...
        # Create some pages
        (temp_wiki / "knowledge" / "concepts" / "test-concept.md").write_text(
            "# Test Concept\n\nContent here."
        )
        (temp_wiki / "technical" / "api" / "endpoints.md").write_text(
            "# API Endpoints\n\nList of endpoints."
        )
    ...
    def test_list_excludes_hidden(self, temp_wiki):
        ...
        # Create visible page
        (temp_wiki / "knowledge" / "concepts" / "visible.md").write_text(
            "# Visible\n\nShould appear."
        )
...
class TestWikiRagFactory:
    def test_get_wiki_rag_defaults_to_local_wiki(self, tmp_path, monkeypatch):
        local_wiki = tmp_path / ".navig" / "wiki"
        local_wiki.mkdir(parents=True, exist_ok=True)
        monkeypatch.chdir(tmp_path)

        rag = get_wiki_rag()
        assert rag.wiki_path.resolve() == local_wiki.resolve()

    def test_get_wiki_rag_uses_project_root_when_provided(self, tmp_path):
        rag = get_wiki_rag(project_root=tmp_path)
        assert rag.wiki_path == tmp_path / ".navig" / "wiki"

    def test_get_wiki_rag_finds_parent_project_wiki_from_nested_cwd(self, tmp_path, monkeypatch):
        ...
        rag = get_wiki_rag()
        assert rag.wiki_path.resolve() == project_wiki.resolve()
...
class TestInboxProcessing:
    ...
    def test_process_inbox_image_with_ocr(self, temp_wiki, monkeypatch):
        """Image inbox items should use OCR content for categorization."""
        init_wiki(temp_wiki)

        image_path = temp_wiki / "inbox" / "receipt.png"
        image_path.write_bytes(b"fake-image-bytes")

        monkeypatch.setattr(
            "navig.core.ocr.extract_ocr_text_from_image_bytes",
            lambda _b: "Invoice total is 42 USD",
        )

        result = process_inbox_item(temp_wiki, "receipt.png", auto_move=False)

        assert "error" not in result
        assert result["file"] == "receipt.png"
        assert "Invoice total" in result["content_preview"]

    def test_process_inbox_image_without_ocr_text_returns_error(self, temp_wiki, monkeypatch):
        """Image inbox items with empty OCR must fail with explicit error."""
        init_wiki(temp_wiki)

        image_path = temp_wiki / "inbox" / "empty.png"
        image_path.write_bytes(b"fake-image-bytes")
        ...
        result = process_inbox_item(temp_wiki, "empty.png", auto_move=False)

        assert "error" in result
        assert "No OCR text detected" in result["error"]
