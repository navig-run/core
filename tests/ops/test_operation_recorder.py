...
# ---------------------------------------------------------------------------
# Fixture for recorder-based tests (merged from root)
# ---------------------------------------------------------------------------

@pytest.fixture()
def recorder(tmp_path):
    """OperationRecorder backed by a temp directory."""
    from navig.operation_recorder import OperationRecorder
    return OperationRecorder(history_dir=tmp_path, max_entries=100)


# ---------------------------------------------------------------------------
# Enum tests (merged from root)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Recorder basics (merged from root)
# ---------------------------------------------------------------------------
...
class TestOperationRecorderExport:
    ...
    def test_export_csv(self, recorder, tmp_path):
        ...
        assert count == 1
        assert "timestamp" in out.read_text()
