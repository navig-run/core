...
def test_lazy_console_supports_context_manager_protocol():
    """_LazyConsole should delegate __enter__/__exit__ to the real Rich console."""
    import navig.console_helper as ch
    ...
    lazy = ch._LazyConsole()
    object.__setattr__(lazy, "_real", FakeConsole())

    with lazy as ctx:
        assert isinstance(ctx, FakeConsole)
