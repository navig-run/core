...
@pytest.mark.asyncio
async def test_start_http_server_registers_routes_and_deck(gateway, monkeypatch: pytest.MonkeyPatch):
    gw, sv = gateway
    gw.config_manager.global_config["telegram"] = {"bot_token": "token", "allowed_users": [1], "require_auth": True}
...
async def test_process_queue_and_message(gateway):
    ...
    await gw._process_message(
        {"channel": "telegram", "user_id": "u1", "message": "hello", "metadata": {"x": 1}, "callback": callback}
    )
...
async def test_cors_and_core_handlers(gateway, monkeypatch: pytest.MonkeyPatch):
    ...
    gw.cron_service = SimpleNamespace(jobs={"j1": SimpleNamespace(enabled=True), "j2": SimpleNamespace(enabled=False)})
...
async def test_message_event_session_and_ws_handlers(gateway):
    ...
    ok = DummyRequest(payload={"channel": "telegram", "user_id": "u", "message": "hi", "metadata": {"a": 1}})
    ...
    evt_ok = await gw._handle_event(DummyRequest(payload={"text": "wake", "agent_id": "n1", "wake_now": True}))
    ...
    await gw._handle_ws_message(ws, {"action": "message", "channel": "ws", "user_id": "u", "message": "hello"})
...
async def test_heartbeat_and_cron_handlers(gateway):
    ...
    added = await gw._handle_cron_add(DummyRequest(payload={"name": "job", "schedule": "* * * * *", "command": "echo hi"}))
    assert response_json(added)["id"] == "job"
    assert (await gw._handle_cron_get(DummyRequest(match_info={"job_id": "missing"}))).status == 404
    assert response_json(await gw._handle_cron_get(DummyRequest(match_info={"job_id": "ok"})))["id"] == "ok"
    assert (await gw._handle_cron_delete(DummyRequest(match_info={"job_id": "missing"}))).status == 404
    assert response_json(await gw._handle_cron_delete(DummyRequest(match_info={"job_id": "ok"})))["success"] is True
    assert (await gw._handle_cron_enable(DummyRequest(match_info={"job_id": "missing"}))).status == 404
    assert response_json(await gw._handle_cron_enable(DummyRequest(match_info={"job_id": "ok"})))["success"] is True
    assert (await gw._handle_cron_disable(DummyRequest(match_info={"job_id": "missing"}))).status == 404
    assert response_json(await gw._handle_cron_disable(DummyRequest(match_info={"job_id": "ok"})))["success"] is True
    assert response_json(await gw._handle_cron_run(DummyRequest(match_info={"job_id": "ok"})))["success"] is True
...
async def test_autonomous_init_and_comms(gateway, monkeypatch: pytest.MonkeyPatch):
    ...
    browser_mod.BrowserController = lambda cfg: SimpleNamespace(_browser=None, _page=None, config=cfg)
    ...
    allowed = await gw.task_worker.handlers["run_command"]({"command": "navig status", "timeout": 5})
    ...
    class ChannelRegistry:
        @staticmethod
        def instance():
            return SimpleNamespace(get_adapter=lambda _name: SimpleNamespace(_notifier="telegram-notifier"))
...
@pytest.mark.asyncio
async def test_feature_handlers_and_agent_interface(gateway, monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    gw, sv = gateway
    ...
    assert response_json(await gw._handle_approval_pending(DummyRequest()))["pending"][0]["id"] == "req1"
    assert response_json(
        await gw._handle_approval_request(DummyRequest(payload={"action": "deploy", "description": "d"}))
    )["request_id"] == "req1"
    assert response_json(
        await gw._handle_approval_respond(DummyRequest(payload={"approved": True}, match_info={"request_id": "req1"}))
    )["success"] is True
    ...
    assert response_json(await gw._handle_browser_navigate(DummyRequest(payload={"url": "https://example.com"})))["success"] is True
    assert response_json(await gw._handle_browser_click(DummyRequest(payload={"selector": "#btn"})))["success"] is True
    assert response_json(await gw._handle_browser_fill(DummyRequest(payload={"selector": "#in", "value": "x"})))["success"] is True
    assert response_json(await gw._handle_browser_screenshot(DummyRequest(payload={"full_page": True})))["success"] is True
    ...
    assert response_json(await gw._handle_mcp_clients(DummyRequest()))["clients"][0]["name"] == "local"
    assert response_json(await gw._handle_mcp_tools(DummyRequest()))["tools"][0]["name"] == "tool.echo"
    assert response_json(await gw._handle_mcp_call_tool(DummyRequest(payload={"arguments": {"x": 1}}, match_info={"tool_name": "tool.echo"})))["success"] is True
    assert response_json(await gw._handle_mcp_connect(DummyRequest(payload={"name": "n1", "command": "run"})))["connected"] is True
    assert response_json(await gw._handle_mcp_disconnect(DummyRequest(payload={"name": "n1"})))["success"] is True
    ...
    assert response_json(await gw._handle_tasks_list(DummyRequest(query={"limit": "10"})))["tasks"][0]["id"] == "t1"
    assert response_json(await gw._handle_tasks_add(DummyRequest(payload={"name": "x", "handler": "h"})))["id"] == "t2"
    assert response_json(await gw._handle_tasks_stats(DummyRequest()))["worker"]["active"] == 0
    assert response_json(await gw._handle_tasks_get(DummyRequest(match_info={"task_id": "t1"})))["id"] == "t1"
    assert response_json(await gw._handle_tasks_cancel(DummyRequest(match_info={"task_id": "t1"})))["success"] is True
    ...
    class KnowledgeEntry:
        ...
        def from_dict(cls, data):
            return cls(key=data["key"], content=data["content"], tags=data.get("tags", []), source=data.get("source", "api"))
    ...
    kb = SimpleNamespace(
        ...
        list_by_source=lambda source, limit=50: [KnowledgeEntry(key="k2", content="content", source=source)],
        export_entries=lambda: [{"key": "k3", "content": "content", "tags": [], "source": "api"}],
        upsert=lambda entry, compute_embedding=False: entry,
        text_search=lambda query, limit=10, tags=None: [KnowledgeEntry(key="k4", content="body", tags=tags or [])],
        count=lambda: 4,
    )
    monkeypatch.setattr(gw, "_get_memory_store", lambda: store)
    monkeypatch.setattr(gw, "_get_knowledge_base", lambda: kb)

    assert response_json(await gw._handle_memory_sessions(DummyRequest(query={"limit": "3"})))["sessions"][0]["session_key"] == "s1"
    assert response_json(await gw._handle_memory_history(DummyRequest(match_info={"session_key": "s1"})))["session_key"] == "s1"
    assert response_json(await gw._handle_memory_delete_session(DummyRequest(match_info={"session_key": "s1"})))["success"] is True
    assert response_json(await gw._handle_memory_add_message(DummyRequest(payload={"session_key": "s1", "content": "hi"})))["success"] is True
    assert response_json(await gw._handle_memory_knowledge_list(DummyRequest(query={"tag": "ops"})))["entries"][0]["key"] == "k1"
    assert response_json(await gw._handle_memory_knowledge_add(DummyRequest(payload={"key": "k5", "content": "v"})))["success"] is True
    assert response_json(await gw._handle_memory_knowledge_search(DummyRequest(query={"q": "body"})))["query"] == "body"
    ...
    for name in ["AGENTS.md", "SOUL.md", "USER.md", "TOOLS.md", "HEARTBEAT.md", "MEMORY.md"]:
        (workspace / name).write_text(f"{name} content", encoding="utf-8")
    (workspace / "memory" / f"{datetime.now().strftime('%Y-%m-%d')}.md").write_text("memory today", encoding="utf-8")
