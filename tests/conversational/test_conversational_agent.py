...
async def test_backward_compat_sync_str_callback() -> None:
    """Compat callback with ``str`` annotation gets event.message."""
    messages: list[str] = []

    def compat_callback(msg: str) -> None:
        messages.append(msg)

    agent = _make_agent(on_status_update=compat_callback)
    event = StatusEvent(
...
async def test_backward_compat_async_str_callback() -> None:
    """Compat async callback with ``str`` annotation is wrapped into a coroutine."""
    messages: list[str] = []

    async def compat_callback(msg: str) -> None:
        messages.append(msg)
...
async def test_backward_compat_no_annotation_single_param() -> None:
    """Callback with no annotation and single param is treated as compat shim input."""
    messages: list[str] = []
...
async def test_lifecycle_task_start_emitted_by_execute_plan() -> None:
    ...
    with (
        ...
        patch(
            ...
            new=AsyncMock(return_value=MagicMock(content='{"achieved": true, "confidence": 100}')),
        ),
    ):
...
async def test_lifecycle_step_failed_with_error_metadata() -> None:
    ...
    with (
        patch.object(executor, "_execute_step", new=AsyncMock(side_effect=RuntimeError("boom"))),
        patch(
            "navig.agent.conv.executor.asyncio.to_thread",
            new=AsyncMock(return_value=MagicMock(content='{"achieved": true, "confidence": 100}')),
        ),
...
async def test_lifecycle_step_done_not_emitted_on_failure() -> None:
    ...
    with (
        patch.object(executor, "_execute_step", new=AsyncMock(side_effect=RuntimeError("kaboom"))),
        patch(
            "navig.agent.conv.executor.asyncio.to_thread",
            new=AsyncMock(return_value=MagicMock(content='{"achieved": true, "confidence": 100}')),
        ),
...
async def test_console_renderer_task_done_prints_elapsed() -> None:
    ...
    with patch.object(renderer._console, "print") as mock_print:
        await renderer(StatusEvent(type="task_done", task_id="abc", message="done", timestamp=t1))
...
async def test_step_failed_final_attempt_is_final_true() -> None:
    ...
    with (
        patch.object(executor, "_execute_step", new=AsyncMock(side_effect=RuntimeError("x"))),
        patch(
            "navig.agent.conv.executor.asyncio.to_thread",
            new=AsyncMock(return_value=MagicMock(content='{"achieved": true, "confidence": 100}')),
        ),
