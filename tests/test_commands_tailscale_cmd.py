...
def _peer(hostname="peer1", ip="100.9.9.1", online=True, os_name="linux"):
    p = MagicMock()
    p.hostname = hostname
    p.tailscale_ip = ip
    p.online = online
...
class TestTsStatus:
    ...
    def test_not_available_exits_nonzero(self):
        cls, _ = _mock_ts(_status(available=False, running=False))
        with patch("navig.integrations.tailscale.Tailscale", cls):
            result = runner.invoke(tailscale_app, ["status"])
        assert result.exit_code != 0

    def test_not_running_exits_nonzero(self):
    ...
    def test_json_flag_outputs_parseable_json(self):
        cls, _ = _mock_ts()
        with patch("navig.integrations.tailscale.Tailscale", cls):
            result = runner.invoke(tailscale_app, ["status", "--json-out"])
    ...
    def test_not_available_produces_output(self):
...
class TestTsPing:
    ...
    def test_success_output_contains_peer_or_pong(self):
        cls, _ = _mock_ts(ping_result=True)
        with patch("navig.integrations.tailscale.Tailscale", cls):
            result = runner.invoke(tailscale_app, ["ping", "somepeer"])
...
class TestTsIp:
    ...
    def test_no_peer_arg_optional(self):
