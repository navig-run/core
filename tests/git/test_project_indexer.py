...
class TestIncrementalUpdate:
    ...
    def test_unreadable_file_is_skipped_with_debug_log(
        self, indexer: ProjectIndexer, project_dir: Path, monkeypatch
    ):
        import navig.memory.project_indexer as idx_mod

        indexer.scan()
        target = project_dir / "src" / "auth.ts"
        target.touch()

        original_read_text = Path.read_text

        def _patched_read_text(path_obj: Path, *args, **kwargs):
            if path_obj == target:
        ...
        with patch.object(idx_mod.logger, "debug") as debug_log:
            stats = indexer.update_incremental()

        assert stats["files_checked"] >= 1
        assert any(
            "failed to read" in str(call).lower() for call in debug_log.call_args_list
        )
