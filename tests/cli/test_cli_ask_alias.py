...
def test_global_host_and_app_flag_values_do_not_trigger_nl_chat(monkeypatch, capsys):
    """Global option values must not be misrouted as natural-language chat input."""
    called = {"count": 0}

    import navig.commands.chat as chat_mod

    def _fake_run_ai_chat(*_args, **_kwargs):
        called["count"] += 1

    monkeypatch.setattr(chat_mod, "run_ai_chat", _fake_run_ai_chat)

    code, _out, _err = _invoke_cli(["--host", "prod", "--app", "portal"], capsys)
