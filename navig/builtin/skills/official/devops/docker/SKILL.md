...
2. **Resource Check**: Run `navig docker stats` to check CPU/Memory usage if a container is unresponsive.
3. **Pruning**: Periodically run `navig run "docker system prune -f"` to clear space, but ask user first.
