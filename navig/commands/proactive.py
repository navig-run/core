...
def proactive_status():
    ...
    if calendar_cfg.get("enabled", False):
        ...
        ch.console.print(f"  [dim]○ Calendar: not configured[/dim]")
    ...
    if email_cfg.get("enabled", False):
        ...
        ch.console.print(f"  [dim]○ Email: not configured[/dim]")
...
def proactive_setup(
    ...
    if calendar_type:
        ...
        if calendar_type == "google":
            ...
            proactive["calendar"]["password"] = f"${{CALDAV_PASSWORD}}"
...
def proactive_test(
    ...
    async def _test():
        from navig.agent.proactive import (
            MockCalendar, MockEmail,
            GoogleCalendar, ICSCalendarProvider,
            GmailProvider
        )
