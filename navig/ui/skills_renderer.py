...
logger = logging.getLogger("navig.ui.skills_renderer")
...
def _get_skills_dirs() -> list[Path]:
    ...
    # Minimal fallback when the loader is not importable. (The old `except` branch
    # pointed at navig/store/skills — the BaseStore *source* package plus "/skills",
    # which has never existed. builtin_store_dir() is the only correct resolver.)
    dirs: list[Path] = []
