...
Usage::
    ...
    from navig.ui.output_styles import get_active_style, load_output_styles
...
logger = logging.getLogger("navig.ui.output_styles")
...
def load_output_styles(cwd: Path | None = None) -> list[OutputStyleConfig]:
    ...
    # user-global styles: config_dir()/output-styles by default (which IS ~/.navig for a
    # normal install, so nothing moves — but follows NAVIG_CONFIG_DIR instead of hardcoding
    # the real home, so a second brain reads its OWN styles). An explicit config override wins.
    from navig.platform.paths import config_dir

    _user_default = str(config_dir() / "output-styles")
    user_dir = Path(cfg.get("user_dir", _user_default)).expanduser()
