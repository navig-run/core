...
def media_dir(kind: str) -> Path:
    """Directory for generated media of *kind* (e.g. ``audio``, ``images``, ``videos``).

    New layout: ``<data_dir>/<kind>`` (e.g. ``~/.navig/data/audio``) — consistent
    with the SSOT :func:`data_dir` and its ``NAVIG_DATA_DIR`` override. In the
    default case the legacy ``<config_dir>/<kind>`` (``~/.navig/<kind>``) is
    preserved when it already exists, so media generated by older builds is never
    orphaned. An explicit ``NAVIG_DATA_DIR`` is authoritative — the legacy location
    is not consulted then.
    """
    new_path = data_dir() / kind
    if not os.environ.get("NAVIG_DATA_DIR"):
        legacy = config_dir() / kind
...
def builtin_store_dir() -> Path:
    """Built-in content store bundled with NAVIG (skills, prompts, templates,
    formations, agents, tools, actions).

    It lives **inside the package** (``navig/builtin/``) because that is the only way
    it reaches a wheel: a directory outside the package can be listed in ``MANIFEST.in``
    (sdist) but setuptools' ``package-data`` cannot reach it, so it is dropped when the
    wheel is built. It used to live at ``<repo>/core/store`` — and every published wheel
    therefore shipped with **zero** builtin content, while it worked fine in a dev
    checkout. Keep this directory inside ``navig/`` (read-only at runtime; user content
    is written to :func:`store_dir`, never here).
    """
...
def scripts_dir() -> Path:
    """User automation scripts directory (``config_dir()/scripts``).

    The single home for user scripts — ``navig script`` (list/run/edit/new),
    ``navig evolve script`` (ScriptEvolver), ``navig ahk``, and ``navig mount`` all
    read and write here. Route every scripts-dir access through this helper so the
    location can never diverge again (see #276/#285).
    """
    return config_dir() / "scripts"


def skills_dir() -> Path:
    """User skills directory (``config_dir()/skills``).

    Where the agent reads global skills (``skills_context``) and the generators write
    them (``navig evolve skill`` / ``SkillEvolver``, ``skill_drafter``). Route every
    skills-dir access through this helper (see #281).
    """
    return config_dir() / "skills"


def workflows_dir() -> Path:
    """User automation workflows directory (``config_dir()/workflows``).

    Where ``navig evolve workflow`` (``WorkflowEvolver``) writes and the automation
    engine (``WorkflowEngine.load_workflow``) reads (see #271).
    """
    return config_dir() / "workflows"


def plugins_dir() -> Path:
    """Installed plugins directory (``config_dir()/plugins``).

    The single home for user-installed plugins — ``navig plugin`` and the plugin host
    read/write here. Route plugins-dir access through this helper so it can never diverge.
    """
    return config_dir() / "plugins"


def spaces_dir() -> Path:
    """Global spaces directory (``config_dir()/spaces`` — note the PLURAL).

    Where installed / global spaces live: ``navig space install`` writes here and the
    space resolver reads from here. The directory is ``spaces``, not ``space`` — a
    hand-built singular ``config_dir()/space`` points at a non-existent directory.
    """
    return config_dir() / "spaces"


def wiki_dir() -> Path:
    """GLOBAL wiki directory (``config_dir()/wiki``).

    The user's global knowledge wiki (``navig wiki`` when no project is active,
    ``get_global_wiki_path()``). NOTE: a *project/space-local* wiki lives at
    ``<project>/.navig/wiki`` — a different, scope-dependent directory that this helper
    does NOT resolve (use the scope-aware ``commands.wiki.get_wiki_path`` for that).
    """
