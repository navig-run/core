...
class ServerDiscovery:
    ...
    def _execute_ssh_paramiko(self, command: str) -> tuple[bool, str, str]:
        ...
        try:
            ...
            if self.debug_logger:
                self.debug_logger.log_ssh_result(
                    success=False, output="", error=error_msg, duration_ms=duration_ms
                )
    ...
    def discover_databases(self, progress: bool = True) -> dict[str, Any]:
        ...
        if postgres_info:
            ...
            if progress:
                ch.success(
                    f"  PostgreSQL: {postgres_info['version']} (port {postgres_info['port']})"
                )
    ...
    def _discover_mysql(self) -> dict[str, Any] | None:
        ...
        for cmd in version_commands:
            ...
            if success and stdout:
                ...
                match = re.search(
                    r"Ver\s+[\d.]+\s+Distrib\s+(\d+\.\d+\.\d+)", stdout, re.IGNORECASE
                )
                if match:
        ...
        success, stdout, _ = self._execute_ssh(
            "ss -tlnp 2>/dev/null | grep -E 'mysqld|mariadb' || netstat -tlnp 2>/dev/null | grep -E 'mysqld|mariadb'"
        )
        if success and stdout:
    ...
    def _discover_laravel_apps(self) -> list[str]:
        ...
        success, stdout, _ = self._execute_ssh(
            "find /var/www /home -name 'artisan' -type f 2>/dev/null | head -10"
        )
        if success and stdout:
            apps = []
    ...
    def format_for_config(self, discovered: dict[str, Any]) -> dict[str, Any]:
        ...
        if web_servers:
            ...
            if web["type"] == "nginx":
                config["paths"]["nginx_config"] = web.get(
                    "sites_path", "/etc/nginx/sites-available"
                )
        ...
        for install_path in ["/var/lib/gitea", "/home/git/gitea", "/opt/gitea"]:
            ...
            if success and "exists" in stdout:
                ...
                for subdir in ["repositories", "data", "log", "custom"]:
                    success2, stdout2, _ = self._execute_ssh(
                        f"test -d {install_path}/{subdir} && echo 'exists'"
                    )
