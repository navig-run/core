"""
forge_bridge.py — Dynamic LLM provider registry for navig-core.
...
class ForgeBridgeRegistry:
    ...
    def _bootstrap_from_bridge_grid(self) -> bool:
        ...
        try:
            ...
            name = f"forge-{app}-{slot}"
            url = f"http://127.0.0.1:{bridge_port}/v1"

            self.register(name, url, priority=0)
            import logging
            logging.getLogger(__name__).info(
                "[ForgeBridge] Auto-registered '%s' at %s from bridge-grid.json", name, url
            )
            return True
        except Exception:
            return False
...
_registry: Optional[ForgeBridgeRegistry] = None
_registry_lock = threading.Lock()


def get_forge_bridge_registry() -> ForgeBridgeRegistry:
    global _registry
    if _registry is None:
        with _registry_lock:
            if _registry is None:
                _registry = ForgeBridgeRegistry()
    return _registry


def reset_forge_bridge_registry() -> None:
    """For testing only."""
    global _registry
    with _registry_lock:
        _registry = None
