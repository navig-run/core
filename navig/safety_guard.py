...
DESTRUCTIVE_PATTERNS = re.compile(
    r"(?:"
    r"rm\s+(-[rfRF]+\s+|--recursive|--force)" r"|"
    r"rmdir\s+" r"|"
    r"DROP\s+(TABLE|DATABASE|SCHEMA|INDEX)" r"|"
    r"TRUNCATE\s+(TABLE)?" r"|"
    r"DELETE\s+FROM\s+\w+\s*(;|$|WHERE\s+1)" r"|"
    r"systemctl\s+(stop|disable|mask)\s+" r"|"
    r"service\s+\w+\s+stop" r"|"
    r"kill\s+-9" r"|"
    r"killall\s+" r"|"
    r"pkill\s+-9" r"|"
    r"format\s+[A-Za-z]:" r"|"
...
RISKY_PATTERNS = re.compile(
    r"(?:"
    r"sudo\s+" r"|"
    r"apt\s+(remove|purge)" r"|"
    r"pip\s+uninstall" r"|"
    r"npm\s+uninstall" r"|"
    r"docker\s+(rm|rmi|prune)" r"|"
...
def require_human_confirmation_if_destructive(
    ...
    if not match:
        ...
        if risky:
            logger.info(
                ...
                risky.group(), _truncate(planned_action),
            )
        else:
    ...
    logger.warning(
        ...
        match.group(), _truncate(planned_action), context or "none",
    )
