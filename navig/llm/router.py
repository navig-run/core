...
`get_router`) is the intended single replacement for BOTH this module and
model_router.py — its docstring: "eliminates the split-brain between
llm_router.py and model_router.py". That migration is unfinished: `run_llm`
(→ resolve_llm here) is still the most-wired path. Until it lands, THIS module's
`resolve_llm` remains the canonical mode→model table. New consolidation work
should finish the UnifiedRouter migration, not add a fourth router. See
docs/ai-models-and-modes.md § "The real issue … THREE routers".
──────────────────────────────────────────────────────────────────────────────
"""
...
logger = logging.getLogger("navig.llm.router")
...
# A sensible default model per provider — used ONLY when `ai.default_provider`
# pins a provider that no configured mode maps to, so the override doesn't emit
# an invalid pair like anthropic:gpt-4o (which 404s). None → skip the override.
...
if PYDANTIC_OK:
    ...
    class LLMModesConfig(BaseModel):
        ...
        def to_dict(self) -> dict[str, Any]:
            """Serialize all modes INCLUDING any user-added extra modes/fields
            (the model allows extras) so a persist round-trip doesn't silently
            drop them (serialize(parse(x)) must not lose data)."""
...
# ────────────────────────────────────────────────────────────────
# Mode Detection — canonical impl lives in navig.llm.routing.detect
...
def _has_api_key(provider: str) -> bool:
    ...
    if _resolve_api_key(provider) is not None:
        return True
    # A routable NAVIG connection counts as a credential — e.g. a claude-max
    # OAuth subscription backs 'anthropic' with no API key. Without this the
    # router would treat anthropic as keyless and bounce to the mode fallback.
    try:
        from navig.providers.inference import has_connection_for_provider

        return has_connection_for_provider(provider)
    except Exception:  # noqa: BLE001
        return False
...
class LLMModeRouter:
    ...
    def _load_config(self) -> None:
        ...
        try:
            ...
            # Don't wipe ALL modes because ONE field is bad — salvage per-mode so
            # a single invalid value (e.g. a hand-edited max_tokens: 999999) resets
            # only the offending mode, not the user's other four.
            self._router_config = self._salvage_config(raw if isinstance(raw, dict) else {})

    def _salvage_config(self, raw: dict[str, Any]) -> "LLMRouterConfig":
        """Rebuild the router config mode-by-mode, keeping every mode that
        validates and substituting the default only for the invalid one(s)."""
        base = LLMRouterConfig()  # all defaults
        modes_raw = raw.get("llm_modes")
        if isinstance(modes_raw, dict):
            for name, mode_cfg in modes_raw.items():
                if name not in CANONICAL_MODES:
                    continue
                try:
                    base.llm_modes.set_mode(name, LLMModeConfig.model_validate(mode_cfg))
                except Exception as exc:  # noqa: BLE001
                    logger.warning("llm_modes.%s invalid — reset to default: %s", name, exc)
        unc = raw.get("uncensored_overrides")
        if isinstance(unc, dict):
            try:
                base.uncensored_overrides = UncensoredOverrides.model_validate(unc)
            except Exception:  # noqa: BLE001
                pass
        return base
    ...
    def _pick_model_for_provider(
        self, provider: str, mode: str, current_model: str
    ) -> str | None:
        """Select a model VALID for *provider*, preferring the current mode
        mapping. Returns ``None`` when nothing valid is known — the caller must
        then SKIP the override rather than pair the provider with another
        provider's model (e.g. anthropic:gpt-4o, which 404s)."""
        ...
        # No mode maps to the pinned provider — use its known default, NOT the
        # original provider's model. None → caller skips the override.
    ...
    def get_config(
        ...
        if self._default_provider and resolved.provider != self._default_provider:
            picked = self._pick_model_for_provider(
            ...
            if picked:
                ...
                # Unknown model for the pinned provider — keep the mode's own
                # valid provider:model instead of emitting a guaranteed-invalid pair.
                logger.warning(
                    "ai.default_provider=%s has no valid model for mode %s — keeping %s:%s",
                    self._default_provider, resolved.mode, resolved.provider, resolved.model,
                )
        return resolved
    ...
    def update_mode(
        ...
        Validates ranges HERE because raw attribute assignment bypasses the
        pydantic ``Field`` constraints (``LLMModeConfig`` has no
        ``validate_assignment``). An out-of-range value would otherwise persist
        and then fail ``model_validate`` on the NEXT load, silently RESETTING the
        entire ``llm_router`` block — wiping every mode. Raises ``ValueError`` on
        bad input so the caller fails loudly instead.
        """
        ...
        if max_tokens is not None and not (1 <= int(max_tokens) <= 131072):
            raise ValueError(f"max_tokens must be between 1 and 131072 (got {max_tokens})")
        if provider is not None:
            cfg.provider = provider.strip().lower()  # normalize (raw assign skips validate_provider)
...
MODE_TOOLSET_HINTS: dict[str, list[str]] = {
    "big_tasks": ["core", "devops", "memory", "browser"],
    "coding": ["core", "code", "search"],
    "research": ["search", "wiki", "memory", "browser"],
...
def detect_mode(user_input: str) -> str:
    ...
    Delegates to :func:`navig.llm.routing.detect.detect_mode`.  Call that
    function directly when you need the ``(mode, confidence, reasons)`` tuple.
    """
    from navig.llm.routing.detect import detect_mode as _detect_canonical  # noqa: PLC0415
...
def _fallback_reachable(provider: str, model: str) -> bool:
    """True when a fallback candidate can plausibly be reached right now.

    A local Ollama fallback is only useful if Ollama is actually up **and** has
    the model installed — otherwise auto-hopping there just turns a clean
    "rate limited" into a doomed round-trip. For remote providers a resolvable
    credential (key or routable connection) is enough.
    """
    p = (provider or "").lower()
    if p == "ollama":
        try:
        ...
        if not installed:
            return False
        base = model.split(":")[0]
        return model in installed or base in installed
    return _has_api_key(p)


def get_mode_fallback_spec(mode: str | None, *, require_reachable: bool = True) -> str | None:
    """Return the mode's configured runtime fallback as ``"provider:model"``.

    Reads the ``llm_modes.<mode>.fallback_provider`` / ``fallback_model`` pair so
    the dispatcher can hop to it when the primary model fails mid-call. Returns
    ``None`` when the mode has no usable fallback configured — or, when
    *require_reachable* (default), when that fallback isn't actually reachable
    (e.g. the default ``ollama`` fallback but Ollama isn't running). This keeps a
    cloud user's rate-limit surfacing clean instead of adding a doomed hop.

    Never raises — fallback resolution must not itself break a call. The
    reachability probe may hit the network (Ollama ``/api/tags``, cached ~5 min),
    so callers should invoke this lazily (only when a fallback is actually
    needed), not on every dispatch.
    """
    if not mode:
        return None
    try:
        router = get_llm_router()
        mode_cfg = router.modes.get_mode(router.resolve_mode(mode))
        if mode_cfg is None:
            return None
        fb_model = (getattr(mode_cfg, "fallback_model", "") or "").strip()
        if not fb_model:
            return None
        fb_provider = (
            getattr(mode_cfg, "fallback_provider", "") or getattr(mode_cfg, "provider", "") or ""
        ).strip()
        if not fb_provider:
            return None
        if require_reachable and not _fallback_reachable(fb_provider, fb_model):
            return None
        return f"{fb_provider}:{fb_model}"
    except Exception as exc:  # noqa: BLE001 — fallback lookup is always best-effort
        logger.debug("get_mode_fallback_spec(%s) failed: %s", mode, exc)
        return None
