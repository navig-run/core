...
Usage:
    from navig.llm.types import ModeRouterProtocol, ModelRouterProtocol, LLMClientProtocol
"""
...
logger = logging.getLogger("navig.llm.types")
...
class ModeRouterProtocol(Protocol):
    ...
    Implementations: navig.llm.router.LLMModeRouter
    """
...
class ModelRouterProtocol(Protocol):
    ...
    Implementations:
      - navig.llm.router.LLMModeRouter.get_config()
    ...
    def _create_client(self, provider_name: str, **kwargs: Any) -> Any:
        ...
        try:
            ...
            if not api_key and not oauth_token:
                # Connection-first (claude-max OAuth / stored key), key-store fallback.
            ...
            client = create_client(
                config, api_key=api_key, oauth_token=oauth_token,
