...
class IntentParser:
    ...
    def _is_conversational(self, message: str) -> bool:
        ...
        if len(words) <= 2 and not any(
            kw in msg.lower() for kw in [
                'disk', 'docker', 'restart', 'backup', 'status', 'hosts',
                'cpu', 'memory', 'uptime', 'ping', 'logs', 'tunnel',
                'database', 'tables', 'ssl', 'dns', 'whois', 'btc', 'eth',
            ]
        ):
        ...
        for pattern in self._conversational_patterns:
            if pattern.search(msg):
                ...
                msg_lower = msg.lower()
                command_signals = [
                    'docker', 'container', 'restart', 'backup', 'database', 'db ',
                    'host ', 'server ', 'disk', 'deploy', 'tunnel', 'ssl', 'dns',
                    'run ', 'execute', 'show me', 'list ', 'check ',
                ]
    ...
    def _compile_patterns(self):
        ...
        pattern_defs = [
            # Docker
            (r'\b(show|list|get|display|what)\b.*\b(docker|containers?)\b', 'docker_ps', {}, 0.85),
            (r'\b(docker|container)\s+(ps|list)\b', 'docker_ps', {}, 0.95),
            (r'\blogs?\s+(for|from|of)?\s*(\w+)\b', 'docker_logs', lambda m: {'container': m.group(2)}, 0.80),
            (r'\brestart\s+(the\s+)?(\w+)\s+(container|docker)\b', 'docker_restart', lambda m: {'container': m.group(2)}, 0.85),
            (r'\brestart\s+container\s+(\w+)\b', 'docker_restart', lambda m: {'container': m.group(1)}, 0.90),
            
            # Host Management
            (r'\b(list|show|get)\s+(all\s+)?(hosts?|servers?|machines?)\b', 'hosts', {}, 0.90),
            (r'\b(switch|use|connect)\s+(to\s+)?(\w[\w-]*)\b', 'use_host', lambda m: {'host_name': m.group(3)}, 0.80),
            (r'\b(change|select)\s+(to\s+)?(server|host)\s+(\w[\w-]*)\b', 'use_host', lambda m: {'host_name': m.group(4)}, 0.85),
            
            # System Monitoring
            (r'\b(check|show|get|what\'?s?)\s+(the\s+)?(disk|storage|space)\b', 'disk', {}, 0.85),
            (r'\b(how\s+much|check)\s+(disk\s+)?space\b', 'disk', {}, 0.85),
            (r'\b(check|show|get|what\'?s?)\s+(the\s+)?(memory|ram|mem)\b', 'memory', {}, 0.85),
            (r'\b(how\s+much|check)\s+(free\s+)?(memory|ram|mem)\b', 'memory', {}, 0.85),
            (r'\b(check|show|get|what\'?s?)\s+(the\s+)?(cpu|processor|load)\b', 'cpu', {}, 0.85),
            (r'\bcpu\s+(usage|load)\b', 'cpu', {}, 0.90),
            (r'\b(server\s+)?uptime\b', 'uptime', {}, 0.90),
            (r'\bhow\s+long\s+(has\s+)?(the\s+)?server\s+(been\s+)?running\b', 'uptime', {}, 0.85),
            ...
            (r'\b(list|show|get)\s+tables?\s+(in|from|of)\s+(\w+)\b', 'db_tables', lambda m: {'database': m.group(3)}, 0.90),
            (r'\btables?\s+(in|from|of)\s+(\w+)\b', 'db_tables', lambda m: {'database': m.group(2)}, 0.85),
            
            # Backups
            (r'\b(list|show|get)\s+(all\s+)?backups?\b', 'backup', {'action': 'list'}, 0.90),
            (r'\b(create|make|run)\s+(a\s+)?backup\b', 'backup', {'action': 'create'}, 0.85),
            
            # Tunnels
            (r'\b(list|show)\s+(all\s+)?tunnels?\b', 'tunnel', {'action': 'list'}, 0.90),
            (r'\b(start|open)\s+tunnel\s+(\w+)\b', 'tunnel', lambda m: {'action': 'start', 'tunnel_name': m.group(2)}, 0.85),
            (r'\b(stop|close)\s+tunnel\s+(\w+)\b', 'tunnel', lambda m: {'action': 'stop', 'tunnel_name': m.group(2)}, 0.85),
            
            # Weather
            (r'\bweather\s+(in\s+)?(.+)$', 'weather', lambda m: {'location': m.group(2)}, 0.90),
            ...
            (r'\bprice\s+of\s+(btc|eth|sol|xrp|doge|ada)\b', 'crypto', lambda m: {'symbol': m.group(1).upper()}, 0.90),
            (r'\bhow\s+much\s+is\s+(bitcoin|ethereum|btc|eth)\b', 'crypto', lambda m: _crypto_symbol(m.group(1)), 0.85),
            (r'\b(list|supported)\s+crypto(currencies?)?\b', 'crypto_list', {}, 0.85),
            
            # Currency
            (r'\bconvert\s+(\d+(?:\.\d+)?)\s*([a-z]{3})\s+to\s+([a-z]{3})\b', 'convert', 
             lambda m: {'amount': float(m.group(1)), 'from_currency': m.group(2).upper(), 'to_currency': m.group(3).upper()}, 0.95),
            (r'\b(\d+(?:\.\d+)?)\s*([a-z]{3})\s+(?:to|in)\s+([a-z]{3})\b', 'convert',
             lambda m: {'amount': float(m.group(1)), 'from_currency': m.group(2).upper(), 'to_currency': m.group(3).upper()}, 0.90),
            
            # Time — specific patterns first, then generic
            (r'\b(?:what\s+)?time\s+is\s+it\s+in\s+(\w+)\b', 'time', lambda m: {'timezone': m.group(1)}, 0.85),
            (r'\btime\s+in\s+(\w+)\b', 'time', lambda m: {'timezone': m.group(1)}, 0.85),
            ...
            (r'\broll\s+(a\s+)?d(ice)?(\d+)?\b', 'roll', lambda m: {'sides': int(m.group(3)) if m.group(3) else 6}, 0.90),
            (r'\b(tell\s+me\s+a\s+)?joke\b', 'joke', {}, 0.85),
            (r'\b(inspirational\s+)?quote\b', 'quote', {}, 0.85),
            
            # Reminders
            (r'\bremind\s+me\s+(?:in\s+)?(\d+)\s*(min(?:utes?)?|hours?|h|days?|d)\s+(?:to\s+)?(.+)$', 'remind',
             lambda m: {'duration': int(m.group(1)), 'unit': _normalize_unit(m.group(2)), 'message': m.group(3)}, 0.95),
            (r'\bset\s+(?:a\s+)?reminder\s+(?:for\s+)?(\d+)\s*(min(?:utes?)?|hours?|h|days?|d)\s*[:-]?\s*(.+)$', 'remind',
             lambda m: {'duration': int(m.group(1)), 'unit': _normalize_unit(m.group(2)), 'message': m.group(3)}, 0.90),
            (r'\b(list|show)\s+(my\s+)?reminders?\b', 'reminders', {}, 0.90),
            (r'\bcancel\s+reminder\s+(\d+)\b', 'cancelreminder', lambda m: {'reminder_id': int(m.group(1))}, 0.95),
            
            # Notes
            (r'\b(list|show)\s+(my\s+)?notes?\b', 'notes', {}, 0.90),
            (r'\bnote[:\s]+(.+)$', 'note', lambda m: {'text': m.group(1)}, 0.85),
            (r'\bsave\s+(?:a\s+)?note[:\s]+(.+)$', 'note', lambda m: {'text': m.group(1)}, 0.90),
            
            # SSL
            (r'\b(check\s+)?ssl\s+(for\s+)?(\S+)\b', 'ssl', lambda m: {'domain': m.group(3)}, 0.85),
            (r'\b(certificate|cert)\s+(for\s+)?(\S+)\b', 'ssl', lambda m: {'domain': m.group(3)}, 0.80),
            
            # DNS
            (r'\bdns\s+(lookup\s+)?(\S+)\b', 'dns', lambda m: {'domain': m.group(2)}, 0.90),
            (r'\blookup\s+dns\s+(\S+)\b', 'dns', lambda m: {'domain': m.group(1)}, 0.85),
            
            # WHOIS
            (r'\bwhois\s+(\S+)\b', 'whois', lambda m: {'domain': m.group(1)}, 0.95),
            
            # Calculator
            (r'\bcalc(?:ulate)?\s+(.+)$', 'calc', lambda m: {'expression': m.group(1)}, 0.90),
            (r'\bwhat\s+is\s+(\d+[\s+\-*/^%]+[\d\s+\-*/^%]+)$', 'calc', lambda m: {'expression': m.group(1)}, 0.85),
            ...
            # Run command
            (r'\brun\s+(command\s+)?(.+)$', 'run_command', lambda m: {'command': m.group(2)}, 0.75),
            (r'\bexecute\s+(.+)$', 'run_command', lambda m: {'command': m.group(1)}, 0.75),
        ]
        ...
        if self.enable_ai and self.config:
            try:
                ...
                if result.confidence >= self.confidence_threshold:
                    ...
                    return result
                logger.debug(f"AI confidence {result.confidence:.2f} below threshold {self.confidence_threshold}")
