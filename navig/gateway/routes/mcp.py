...
def _chk(gw):
    if not gw.mcp_client_manager:
        return json_error_response("MCP module not available", status=503, code="module_unavailable")
    return None
...
def _tools(gw):
    async def h(r):
        ...
        return json_ok({
            "tools": [
                {"name": t.name, "description": t.description, "client": t.client_name}
                for t in tools
            ]
        })
    return h
    ...
    async def h(r):
        ...
        try:
            ...
            result = await gw.mcp_client_manager.call_tool(
                tool_name=tool_name, arguments=data.get("arguments", {}),
            )
            return json_ok({"result": result})
        except Exception as e:
            return json_error_response("MCP tool call failed", status=500, code="internal_error", details={"error": str(e)})
    return h
    ...
    async def h(r):
        ...
        try:
            ...
            client = await gw.mcp_client_manager.add_client(
                name=data["name"], command=data.get("command"), url=data.get("url"),
            )
            return json_ok({
                "name": data["name"],
            ...
            return json_error_response("MCP connect failed", status=500, code="internal_error", details={"error": str(e)})
    return h
...
def _disconnect(gw):
    async def h(r):
        ...
        try:
            ...
            return json_error_response("MCP disconnect failed", status=500, code="internal_error", details={"error": str(e)})
    return h
