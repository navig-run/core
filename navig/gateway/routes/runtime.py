...
def _list_missions(gw: "NavigGateway"):
    async def h(r: "web.Request") -> "web.Response":
        ...
        from navig.contracts.store import get_runtime_store
        store = get_runtime_store()
        missions = [m.to_dict() for m in store.list_missions(node_id=node_id, status=status, limit=limit)]
...
def _get_mission(gw: "NavigGateway"):
    async def h(r: "web.Request") -> "web.Response":
        ...
        if mission is None:
            return json_error_response(f"Mission {mission_id!r} not found", status=404, code="not_found")
...
def _advance_mission(gw: "NavigGateway"):
    ...
    async def h(r: "web.Request") -> "web.Response":
        ...
        if not action:
            return json_error_response("'action' field required", status=422, code="validation_error")
...
def _complete_mission(gw: "NavigGateway"):
    ...
    async def h(r: "web.Request") -> "web.Response":
        ...
        store = get_runtime_store()
        try:
            receipt = store.complete_mission(mission_id, succeeded=succeeded, result=output, error=error_msg)
        except KeyError as exc:
            return json_error_response(str(exc), status=404, code="not_found")
        except ValueError as exc:
            return json_error_response(str(exc), status=422, code="invalid_transition")
...
def _list_receipts(gw: "NavigGateway"):
    async def h(r: "web.Request") -> "web.Response":
        ...
        from navig.contracts.store import get_runtime_store
        store = get_runtime_store()
        receipts = [rec.to_dict() for rec in store.list_receipts(node_id=node_id, mission_id=mission_id, limit=limit)]
...
def _get_receipt(gw: "NavigGateway"):
    async def h(r: "web.Request") -> "web.Response":
        ...
        if receipt is None:
            return json_error_response(f"Receipt {receipt_id!r} not found", status=404, code="not_found")
