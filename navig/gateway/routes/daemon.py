...
def _formation_start(gw):
    async def h(request):
        ...
        try:
            subprocess.Popen([sys.executable, "-m", "navig.cli", "formation", "start", formation_name])
            resp = json_ok({"status": "starting", "formation": formation_name})
            resp.headers["Access-Control-Allow-Origin"] = "*"
            return resp
        except Exception as e:
            resp = json_error_response("Failed to start formation", status=500, details={"error": str(e)})
            resp.headers["Access-Control-Allow-Origin"] = "*"
            return resp
